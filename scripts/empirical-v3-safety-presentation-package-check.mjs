...
const processRecord = Object.freeze({
  ...
  fieldStates: Object.freeze({
    operatingTemperature: Object.freeze({ status: 'DECLARED', value: 180, unit: 'degC', required: true }),
  }),
  ref: 'process-basis:fixture',
...
const branch = sealEmpiricalV3BranchAuthority({
  ...
  componentIds: ['P101'],
  commonAuthorityRefs: [{ kind: 'PROCESS', ref: processRecord.ref, semanticHash: processRecord.semanticHash }],
  sourceEvidenceRefs: [],
  riskRefs: [],
});
const component = bindComponentToBranch({
  componentId: 'P101',
  componentType: 'PIPE',
  topologyComponentRef: { ref: 'route-component:P101', semanticHash: 'fnv1a64:4444444444444444' },
  localAuthorityRefs: [{ kind: 'WT', ref: 'Q:P101:WT', semanticHash: 'fnv1a64:5555555555555555' }],
  sourceEvidenceRefs: [],
  riskRefs: [],
}, branch);
...
const workflow = projectEmpiricalV3Workflow({
  source: { bound: true, current: true },
  authorities: { built: true, current: true },
  branches: { built: true, current: true, reviewCurrent: true },
  riskSet: { evaluated: true, current: true, highBlockCount: 0, highConfirmPendingCount: 0 },
  calculationAuthorization: { present: false, current: false, semanticHash: null },
  calculationResult: { present: false, current: false, reviewRequired: true },
  resultReview: { present: false, current: false },
  audit: { ready: false, current: false },
});
...
const input = {
  ...
  riskSet,
  confirmations: [],
  calculationAuthorization: null,
  records: [{
    ref: processRecord.ref,
    kind: 'PROCESS',
    semanticHash: processRecord.semanticHash,
...
assert.throws(() => sealEmpiricalV3SafetyPresentationPackage({
  ...input,
  workflow: { ...workflow, state: 'CALCULATION_AUTHORIZED', canRunCalculation: true, calculationAuthorizationRef: 'fnv1a64:6666666666666666' },
}), /authorization/i, 'UI package cannot claim authorization without a sealed authorization artifact');

console.log('PASS empirical v3 safety presentation package');
