...
export const EMPIRICAL_V3_WORKFLOW_STATES = Object.freeze([
  'NO_SOURCE',
  'SOURCE_READY',
  'AUTHORITY_BUILD_REQUIRED',
  'BRANCH_REVIEW_REQUIRED',
  'SAFETY_REVIEW_REQUIRED',
  'HIGH_BLOCK_PRESENT',
  'HIGH_CONFIRM_PENDING',
  'SAFETY_CLEARED',
...
export const EMPIRICAL_V3_WORKFLOW_ACTIONS = Object.freeze({
  BUILD_AUTHORITIES: 'BUILD_AUTHORITIES',
  REVIEW_BRANCH_BASIS: 'REVIEW_BRANCH_BASIS',
  REVIEW_SAFETY: 'REVIEW_SAFETY',
  REVIEW_HIGH_CONFIRM: 'REVIEW_HIGH_CONFIRM',
  RUN_CALCULATION: 'RUN_CALCULATION',
  REQUIRE_RESULT_REVIEW: 'REQUIRE_RESULT_REVIEW',
...
const ACTION_STATES = Object.freeze({
  ...
  REVIEW_HIGH_CONFIRM: new Set(['HIGH_CONFIRM_PENDING']),
  RUN_CALCULATION: new Set(['CALCULATION_AUTHORIZED']),
  REQUIRE_RESULT_REVIEW: new Set(['CALCULATED']),
  REVIEW_RESULT: new Set(['RESULT_REVIEW_REQUIRED']),
  EXPORT_AUDIT: new Set(['AUDIT_EXPORT_READY']),
});
...
export function projectEmpiricalV3Workflow(input = {}) {
  ...
  const identityFields = {
    schema: EMPIRICAL_V3_WORKFLOW_SCHEMA,
    state,
    facts,
    reasonCodes,
  };
  ...
  return deepFreeze({
    ...
    calculationAuthorizationRef:
      state === 'CALCULATION_AUTHORIZED' ? facts.calculationAuthorization.semanticHash : null,
...
export function canPerformEmpiricalV3WorkflowAction(projection, action) {
  ...
  return allowedStates.has(projection.state);
}

export function requireEmpiricalV3WorkflowAction(projection, action) {
  if (!canPerformEmpiricalV3WorkflowAction(projection, action)) {
    throw new Error(`Empirical V3 action ${action} is not permitted from ${projection.state}.`);
  }
  return projection;
}
...
function selectState(facts) {
  ...
  if (!facts.branches.built || !facts.branches.current || !facts.branches.reviewCurrent) {
    return 'BRANCH_REVIEW_REQUIRED';
  }

  if (!facts.riskSet.evaluated || !facts.riskSet.current) return 'SAFETY_REVIEW_REQUIRED';
  if (facts.riskSet.highBlockCount > 0) return 'HIGH_BLOCK_PRESENT';
  if (facts.riskSet.highConfirmPendingCount > 0) return 'HIGH_CONFIRM_PENDING';

  if (!facts.calculationAuthorization.present || !facts.calculationAuthorization.current) {
    return 'SAFETY_CLEARED';
  }

  if (!facts.calculationResult.present || !facts.calculationResult.current) {
    return 'CALCULATION_AUTHORIZED';
  }
...
function reasonsForState(state, facts) {
  switch (state) {
    case 'NO_SOURCE': return ['SOURCE_NOT_BOUND'];
    case 'SOURCE_READY': return ['SOURCE_REBIND_OR_CURRENTNESS_REQUIRED'];
    case 'AUTHORITY_BUILD_REQUIRED': return ['AUTHORITY_SET_MISSING_OR_STALE'];
    case 'BRANCH_REVIEW_REQUIRED': return ['BRANCH_BASIS_MISSING_STALE_OR_UNREVIEWED'];
    case 'SAFETY_REVIEW_REQUIRED': return ['RISK_SET_MISSING_OR_STALE'];
    case 'HIGH_BLOCK_PRESENT': return [`HIGH_BLOCK_COUNT:${facts.riskSet.highBlockCount}`];
    case 'HIGH_CONFIRM_PENDING': return [`HIGH_CONFIRM_PENDING_COUNT:${facts.riskSet.highConfirmPendingCount}`];
    case 'SAFETY_CLEARED': return ['CALCULATION_AUTHORIZATION_MISSING_OR_STALE'];
    case 'CALCULATION_AUTHORIZED': return ['CURRENT_SEALED_CALCULATION_AUTHORIZATION'];
    case 'CALCULATED': return ['CURRENT_RESULT_AWAITS_REVIEW_REQUIREMENT'];
    case 'RESULT_REVIEW_REQUIRED': return ['CURRENT_RESULT_REVIEW_REQUIRED'];
    case 'RESULT_REVIEWED': return ['CURRENT_RESULT_REVIEWED_AUDIT_NOT_READY'];
    case 'AUDIT_EXPORT_READY': return ['CURRENT_AUDIT_EXPORT_READY'];
    default: throw new RangeError(`Unsupported Empirical V3 workflow state: ${state}`);
  }
}
...
function normalizeVerifiedFacts(input) {
  ...
  return deepFreeze({
    ...
    riskSet: {
      ...riskSet,
      highBlockCount: requireNonNegativeInteger(input.riskSet?.highBlockCount ?? 0, 'riskSet.highBlockCount'),
      highConfirmPendingCount: requireNonNegativeInteger(
        input.riskSet?.highConfirmPendingCount ?? 0,
        'riskSet.highConfirmPendingCount',
      ),
    },
    calculationAuthorization,
    calculationResult,
...
function normalizeRecord(value, booleanFields, stringFields = []) {
...
function requireProjection(value) {
  if (!value || value.schema !== EMPIRICAL_V3_WORKFLOW_SCHEMA) {
    throw new TypeError('A current Empirical V3 workflow projection is required.');
  }
}
