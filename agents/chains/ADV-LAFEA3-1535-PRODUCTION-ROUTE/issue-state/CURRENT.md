...
ISSUE_LATEST_ENDPOINT_COMMENT_ID: 5470192926
...
| INPUT-005 | UNRESOLVED | Hosted runner allocation fails before step 1 even on exact main; prior direct local checkout also failed network/DNS. |
...
| BM-005 | NOT_RUN_EXECUTION_BLOCKED | Exact-main push run exists, but both exact-main hosted attempts terminate before checkout; no harness stdout/report artifact exists. |
...
The first unexecuted boundary is `actions/checkout@v4`. Therefore no clean-tree guard, syntax check, BM-005 harness, mesh, solver, recovery, physical probe, convergence, report or negative control executed. The Actions `failure` conclusion is infrastructure evidence only and must not be converted into a BM-005 numerical FAIL.
...
No changes are authorized to element formulation, stiffness/load assembly, solver/recovery, mesher mathematics/quality thresholds, pressure semantics, physical-probe mathematics, convergence mathematics/policy, frozen benchmark oracle/mesh/probe/tolerances, report semantics, UI/browser semantics, roadmap content, code assessment or release authority while the first wrong boundary remains hosted runner allocation.
...
Q1 exact-main production trace and NOT_RUN classification; Q2 four-level Richardson/GCI/asymptotic proof; Q3 independent Lamé oracle/pressure sign/negative control; Q4 report/hash/exact-head authority; Q5 first-wrong-boundary/minimal-patch/BM-006 gate.
...
Obtain a functioning hosted runner or faithful clean local checkout of exact main `26f732ba8142828dc6c6160e4355993a60aa186a`, then run `node scripts/lafea.3-bm005-ordinary-route-check.mjs`. Preserve exact stdout/stderr/exit and candidate HEAD. PASS -> persist exact report then BM-006 real-browser replay. FAIL/pre-report rejection -> isolate first wrong boundary and create fresh pre-work before any engineering patch. Do not add another workflow merely to work around the runner outage.
