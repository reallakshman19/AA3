...
/** Legacy comparison-only state retained for direct historical callers. */
export const EMP1_WRC537_ATTACHMENT_SOURCE_QUALIFICATION =
  'UNQUALIFIED_FOR_PRODUCTION';

/**
 * Retain real LAFEA.2 screening plus attachment geometry. Product execution may
 * supply a typed, hash-qualified attachmentSourceAuthority. Historical direct
 * callers may still provide diameter + locator, but that evidence remains
...
export function createEmp1RetainedSectionScreeningLayer({
  ...
  const attachmentGeometryEvidence = attachmentSourceAuthority
    ? requireEmp1Wrc537QualifiedAttachmentSourceAuthority(attachmentSourceAuthority)
    : createLegacyAttachmentGeometryEvidence({
      geometryIdentity,
      outsideDiameter: attachmentDiameter,
      sourceReference: attachmentSourceReference,
    });
  const boundGeometryIdentity = requiredString(
    attachmentGeometryEvidence.geometryIdentity ?? geometryIdentity,
    'EMP1_WRC537_ATTACHMENT_GEOMETRY_IDENTITY_REQUIRED',
  );
  const geometryEvidence = createCorrelationGeometryEvidenceFromLafea2({
    screeningRequest,
    screeningResult,
    geometryIdentity: boundGeometryIdentity,
...
export function deriveEmp1Wrc537SourceCustody({
  ...
  const attachmentEvidence = requireAttachmentGeometryEvidence(
    screening.attachmentGeometryEvidence,
  );
  ...
  const loadCase = foundation.transformedLoadCases?.find(
    (row) => row.identity === loadCaseIdentity,
  );
  if (!loadCase) throw custodyError('EMP1_WRC537_SOURCE_LOAD_CASE_NOT_FOUND');
  const targetPoint = vector3(
    loadCase.targetPointGlobal,
    'EMP1_WRC537_SOURCE_TARGET_REFERENCE_INVALID',
  );
  const axesEvidence = foundation.coordinateSystemEvidence;
  const vesselAxis = vector3(
    axesEvidence?.axesGlobal?.eX,
    'EMP1_WRC537_SOURCE_VESSEL_AXIS_INVALID',
  );
  const foundationRadialAxis = vector3(
    axesEvidence?.axesGlobal?.eZ,
    'EMP1_WRC537_SOURCE_NOZZLE_AXIS_INVALID',
  );
  const shellThickness = positive(
    geometryEvidence.pipeThickness,
    'EMP1_WRC537_SOURCE_SHELL_THICKNESS_INVALID',
  );
  const outerRadius = positive(
    geometryEvidence.pipeOutsideDiameter,
    'EMP1_WRC537_SOURCE_PIPE_OD_INVALID',
  ) / 2;
  ...
  const attachmentOutsideRadius = positive(
    attachmentEvidence.outsideDiameter,
    'EMP1_WRC537_SOURCE_ATTACHMENT_OUTSIDE_DIAMETER_INVALID',
  ) / 2;
  ...
  return deepFreeze({
    ...
    attachmentGeometryEvidence: structuredClone(attachmentEvidence),
    geometry: {
      ...
      derivation: {
        ...
        attachmentOutsideRadius:
          'WRC_ATTACHMENT_OUTSIDE_DIAMETER_OVER_2_AT_SHELL_JUNCTURE',
        gamma: 'Rm/T',
        beta: '0.875*r0/Rm',
      },
    },
    geometrySourceReferences: {
      ...
      attachmentPhysicalLocation: attachmentEvidence.physicalLocation,
      attachmentSourceBindingSemanticHash:
        attachmentEvidence.sourceBindingSemanticHash ?? null,
    },
    loadReference: {
      identity: requiredString(
        loadCase.targetReferencePointIdentity,
    ...
    axes: {
      ...
      coordinateSystemIdentity: requiredString(
        axesEvidence?.identity,
...
export function requireEmp1Wrc537SourceCustody(value) {
  ...
  requiredString(
    value.attachmentGeometryEvidenceHash,
  ...
  if (attachment.semanticHash !== value.attachmentGeometryEvidenceHash) {
    throw custodyError('EMP1_WRC537_SOURCE_ATTACHMENT_GEOMETRY_HASH_MISMATCH');
  }
  ...
  if (value.geometry?.attachmentRadiusSourceQualification !== attachment.sourceQualification) {
    throw custodyError('EMP1_WRC537_SOURCE_R0_QUALIFICATION_MISMATCH');
  }
...
export function requireEmp1Wrc537QualifiedR0SourceCustody(value) {
  const custody = requireEmp1Wrc537SourceCustody(value);
  const attachment = requireEmp1Wrc537QualifiedAttachmentSourceAuthority(
    custody.attachmentGeometryEvidence,
  );
  if (custody.geometry.attachmentRadiusSourceQualification
    !== EMP1_WRC537_ATTACHMENT_SOURCE_QUALIFIED) {
    throw custodyError('EMP1_WRC537_SOURCE_R0_NOT_QUALIFIED_FOR_PRODUCTION');
  }
  if (custody.geometry.attachmentOutsideRadius !== attachment.outsideDiameter / 2) {
    throw custodyError('EMP1_WRC537_SOURCE_R0_VALUE_DRIFT');
  }
  return custody;
}

function createLegacyAttachmentGeometryEvidence({
  geometryIdentity,
  outsideDiameter,
  sourceReference,
}) {
...
function requireAttachmentGeometryEvidence(value) {
  if (value?.schema === EMP1_WRC537_ATTACHMENT_SOURCE_AUTHORITY_SCHEMA) {
    return requireEmp1Wrc537QualifiedAttachmentSourceAuthority(value);
  }
  if (!record(value)
    || value.schema !== EMP1_WRC537_ATTACHMENT_GEOMETRY_EVIDENCE_SCHEMA) {
...
function requireRetainedScreeningLayer(value) {
  if (!record(value)
    || value.schema !== EMP1_WRC537_RETAINED_SCREENING_LAYER_SCHEMA
  ...
  const attachment = requireAttachmentGeometryEvidence(value.attachmentGeometryEvidence);
  if (retained.geometryIdentity !== attachment.geometryIdentity
    || retained.attachmentDiameter !== attachment.outsideDiameter
...
function requireQualifiedFoundationResult(value) {
  if (!record(value)
    || value.schema !== 'local-attachment-foundation-result/v1'
