...
import {
  requireEmp1Wrc537ComparisonLoadCustody,
  requireEmp1Wrc537QualifiedLoadCustody,
} from './emp1-wrc537-load-custody.js';
import {
...
const FIGURE_BASE = deepFreeze({
  circ: {
    Pmem_AB: '4C', Pmem_CD: '3C', Pbend_AB: '2C-1', Pbend_CD: '1C',
    Mcmem: '3A', Mcbend: '1A', Mlmem: '3B',
  },
  long: {
    Pmem_AB: '3C', Pmem_CD: '4C', Pbend_AB: '1C-1', Pbend_CD: '2C',
    Mcmem: '4A', Mcbend: '2A', Mlmem: '4B',
  },
});
const UNIT_SYSTEMS = deepFreeze({
  SI_MM: { force: 'N', length: 'mm', moment: 'N*mm', stress: 'N/mm^2' },
  US_IN: { force: 'lbf', length: 'in', moment: 'lbf*in', stress: 'psi' },
});
...
export function evaluateEmp1Wrc537CylindricalBoundedAdapter(input) {
  ...
  return evaluateWithCustody(
    input,
    custody,
    'EVALUATED_BOUNDED_GAMMA5_TABLE5_COMPARISON',
    false,
  );
}

export function evaluateEmp1Wrc537CylindricalBoundedQualifiedNumerics(
  input,
  { expectedProducerQualificationHash } = {},
) {
  const custody = requireEmp1Wrc537QualifiedLoadCustody(
    input?.loadCustody,
    { expectedProducerQualificationHash },
  );
  return evaluateWithCustody(
    input,
    custody,
    'EVALUATED_BOUNDED_GAMMA5_TABLE5_QUALIFIED_INPUT',
    true,
  );
}

export function deriveEmp1Wrc537CylindricalBoundedGeometry(value) {
  return deriveGeometry(value, true);
}

function evaluateWithCustody(input, loadCustody, state, qualifiedInputAuthority) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw adapterError('EMP1_WRC537_BOUNDED_ADAPTER_INPUT_REQUIRED');
  }
  const units = normalizeUnits(input.units);
  const geometry = deriveGeometry(input.geometry, qualifiedInputAuthority);
  const longitudinalAuthority = qualifiedInputAuthority
    ? requireEmp1Wrc537Table5EightPointLongitudinalMomentAuthority(
      input.longitudinalMomentBendingAuthority,
    )
    : null;
  const lmSelection = longitudinalAuthority
    ? deepFreeze(structuredClone(longitudinalAuthority.selection))
    : resolveEmp1Wrc537LongitudinalMomentBendingSelection(
      input.longitudinalMomentBendingSelection,
    );
  const figures = figureMap(lmSelection);
  ...
  const domain = requireEmp1Wrc537BoundedDomain({
  ...
  const table5 = evaluateEmp1Wrc537CylindricalTable5({
    ...
    loads: wrcLoads,
    curveOrdinates: curveEvaluation.ordinates,
  });
  return deepFreeze({
    ...
    frame,
    wrcLoads,
    longitudinalMomentBendingAuthority: longitudinalAuthority,
    longitudinalMomentBendingSelection: lmSelection,
    curveFigureMap: figures,
    curveSelections: curveEvaluation.selections,
    curveOrdinates: curveEvaluation.ordinates,
    table5,
...
function figureMap(selection) {
  return deepFreeze({
    circ: { ...FIGURE_BASE.circ, Mlbend: selection.circumferentialFigure },
    long: { ...FIGURE_BASE.long, Mlbend: selection.longitudinalFigure },
  });
}

function deriveGeometry(value, requireOutsideRadiusAuthority = false) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw adapterError('EMP1_WRC537_BOUNDED_GEOMETRY_REQUIRED');
  }
  const meanRadius = positive(value.meanRadius, 'MEAN_RADIUS');
  const shellThickness = positive(value.shellThickness, 'SHELL_THICKNESS');
  const hasOutside = value.attachmentOutsideRadius != null;
  const hasLegacy = value.attachmentRadius != null;
  if (requireOutsideRadiusAuthority && !hasOutside) {
    throw adapterError('EMP1_WRC537_BOUNDED_ATTACHMENT_OUTSIDE_RADIUS_REQUIRED');
  }
  if (!hasOutside && !hasLegacy) {
    throw adapterError('EMP1_WRC537_BOUNDED_ATTACHMENT_OUTSIDE_RADIUS_REQUIRED');
  }
  const attachmentOutsideRadius = positive(
    hasOutside ? value.attachmentOutsideRadius : value.attachmentRadius,
    hasOutside ? 'ATTACHMENT_OUTSIDE_RADIUS' : 'ATTACHMENT_RADIUS',
  );
  if (hasOutside && hasLegacy
    && !roundOffEquivalent(value.attachmentRadius, attachmentOutsideRadius)) {
    throw adapterError('EMP1_WRC537_BOUNDED_ATTACHMENT_RADIUS_ALIAS_MISMATCH');
  }
  const attachmentRadiusBasis = hasOutside
    ? EMP1_WRC537_R0_BASIS
    : 'LEGACY_UNQUALIFIED_ATTACHMENT_RADIUS';
  const gamma = meanRadius / shellThickness;
  const beta = 0.875 * attachmentOutsideRadius / meanRadius;
  if (value.gamma != null && !roundOffEquivalent(value.gamma, gamma)) {
    throw adapterError('EMP1_WRC537_BOUNDED_DECLARED_GAMMA_MISMATCH');
  }
  if (value.beta != null && !roundOffEquivalent(value.beta, beta)) {
    throw adapterError('EMP1_WRC537_BOUNDED_DECLARED_BETA_MISMATCH');
  }
  return deepFreeze({
    meanRadius,
    shellThickness,
    attachmentOutsideRadius,
    attachmentRadius: attachmentOutsideRadius,
    attachmentRadiusBasis,
    gamma,
    beta,
    gammaEquation: 'Rm/T',
    betaEquation: '0.875*r0/Rm',
    r0Definition: 'OUTSIDE_RADIUS_OF_CYLINDRICAL_ATTACHMENT_AT_SHELL_JUNCTURE',
    declaredGamma: value.gamma ?? null,
    declaredBeta: value.beta ?? null,
  });
}

function evaluateFigureSet({ figures, variant, gamma, beta }) {
  ...
  for (const family of ['circ', 'long']) {
    for (const [quantity, figure] of Object.entries(figures[family])) {
      ...
      if (evaluation.y < 0) {
        throw adapterError(`EMP1_WRC537_BOUNDED_NEGATIVE_ORDINATE:${figure}:${evaluation.y}`);
      }
      selections[family][quantity] = deepFreeze({
        figure: curve.figure,
        variant: curve.variant,
        sourceGamma: curve.gamma,
        pdfPage: curve.pdfPage,
        sourceDocumentSha256: curve.sourceDocumentSha256,
        datasetHash: curve.datasetHash,
        interpolationUsed: curve.interpolationUsed,
        extrapolationFallbackUsed: curve.extrapolationFallbackUsed,
      });
...
function normalizeUnits(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw adapterError('EMP1_WRC537_BOUNDED_UNITS_REQUIRED');
  }
  for (const [system, contract] of Object.entries(UNIT_SYSTEMS)) {
    if (Object.entries(contract).every(([key, expected]) => value[key] === expected)) {
      return deepFreeze({ system, ...contract });
    }
  }
...
function positive(value, label) {
  if (!Number.isFinite(value) || value <= 0) {
    throw adapterError(`EMP1_WRC537_BOUNDED_${label}_INVALID`);
  }
  return Number(value);
}
function requiredString(value, label) {
  if (typeof value !== 'string' || !value.trim()) {
    throw adapterError(`EMP1_WRC537_BOUNDED_${label}_REQUIRED`);
  }
  return value.trim();
}
function roundOffEquivalent(actual, source) {
  return Number.isFinite(actual)
    && Math.abs(actual - source)
      <= Math.max(1, Math.abs(source)) * ROUND_OFF_RELATIVE_TOLERANCE;
}
