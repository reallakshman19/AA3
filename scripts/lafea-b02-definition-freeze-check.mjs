...
assert.equal(manifest.adoptedG4ParentExactHead, 'c3fb24b5fc354763e5fd8f86161da14ac228e80b');
assert.equal(manifest.g4ParentGate, 'PASS');
assert.equal(manifest.frozenBeforeAnyB02ProductionObservation, true);
assert.equal(manifest.productionOutputUsedToGenerateTargetsTolerancesProbesOrMeshes, false);
...
execFileSync('git', ['merge-base', '--is-ancestor', manifest.adoptedG4ParentExactHead, 'HEAD'], { cwd: ROOT });
...
for (const id of ['B02A', 'B02B', 'B02C']) assertRatio2(definitions[id].meshLadder.levels);
assertRatio2(definitions.B02D.globalResponseLadder.levels);
assert.deepEqual(definitions.B02D.globalResponseLadder.levels.map((row) => row.expectedElements.T6), [64, 256, 1024, 4096]);
...
for (const binding of Object.values(e.subBucketBindings)) assertRatio2(binding.levelIds.map((levelId, index) => ({ levelId, h: binding.h[index] })));
...
assert.notEqual(definitions.B02B.title.toUpperCase().includes('PURE SHEAR PATCH'), true);
...
const hashes = Object.fromEntries(Object.entries(manifest.definitionFiles).map(([id, relative]) => {
  const bytes = fs.readFileSync(path.join(ROOT, relative));
  return [id, `sha256:${crypto.createHash('sha256').update(bytes).digest('hex')}`];
}));
console.log(JSON.stringify({
  schema: 'lafea-b02-definition-freeze-receipt/v1',
  status: 'PASS',
  adoptedG4ParentExactHead: manifest.adoptedG4ParentExactHead,
...
function validateProbeSet(probes) {
  ...
  for (const probe of probes) {
    for (const key of ['probeId','physicalCoordinate','coordinateFrame','loadCaseId','quantityId','representation','recoveryMethod','units','singularityClassification']) assert.ok(probe[key] !== undefined, `${probe.probeId} missing ${key}`);
