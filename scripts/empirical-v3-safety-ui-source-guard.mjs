...
assert.match(safety,/risk\.riskClass === 'HIGH_CONFIRM'/); assert.match(safety,/BLOCKED — no confirmation path/); assert.match(safety,/Create confirmation receipt/); assert.match(safety,/packageValue\.riskSet\.risks/); assert.match(safety,/workflow\.canRunCalculation/);
...
assert.match(explain,/evidence\.equations/); assert.match(explain,/evidence\.coupledSystem\.flexibilityMatrixMPerN/); assert.match(explain,/coordinate\.pairEvidence/); assert.match(explain,/componentContributions/); assert.match(explain,/evidencePolicy\.uiOrReportMayResolveMechanics/);
assert.doesNotMatch(explain,/\.reduce\(|reactionN\s*[+*/-]|flexibilityRowMPerN\s*[+*/-]/);
const reviewView=sourceAt('src/workspace/empirical-v3-result-review-view.js'); assert.match(reviewView,/Record result review/); assert.match(reviewView,/does not change reactions, displacements, risks, or formulas/);
const reviewController=sourceAt('src/workspace/empirical-v3-review-audit-controller.js');
assert.match(reviewController,/createEmpiricalV3ResultReviewReceipt/); assert.match(reviewController,/sealEmpiricalV3AuditReadiness/); assert.match(reviewController,/workflowState !== 'AUDIT_EXPORT_READY'/); assert.match(reviewController,/createEmpiricalV3AuditJsonExport/);
const controller=sourceAt('src/workspace/empirical-v3-safety-workbench.js');
assert.match(controller,/createEngineeringConfirmationReceipt/); assert.match(controller,/Safety Gate remains unchanged until a re-evaluated sealed package is supplied/); assert.match(controller,/VIEWPORT_SELECTION_REQUESTED/); assert.match(controller,/onRunRequested/); assert.match(controller,/requireEmpiricalV3CoupledCalculationEvidence/);
assert.match(controller,/workflow\.state !== 'RESULT_REVIEW_REQUIRED'/); assert.match(controller,/Audit remains gated by AUDIT_EXPORT_READY workflow/); assert.match(controller,/evidence\.authorizationRef\.semanticHash === packageValue\.calculationAuthorization\.semanticHash/);
const main=sourceAt('src/main.js'); assert.match(main,/mountEmpiricalV3SafetyWorkbench/); assert.match(main,/loadEmpiricalV3SafetyPresentationPackage/); assert.match(main,/executeEmpiricalV3SourceBoundThermalRom/); assert.match(main,/buildEmpiricalV3SourceBoundExecutionDependency/); assert.match(main,/createEmpiricalV3AuditExportRecord/);
...
console.log('PASS empirical v3 safety / Explain / result-review UI source guard');
