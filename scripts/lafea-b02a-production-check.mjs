...
const definition = readJson('validation/lafea-b02-definitions/B02A-nonuniform-bending.json');
const convergencePolicy = readJson('validation/lafea-b02-definitions/B02E-convergence.json');

assert.equal(definition.caseId, 'B02A');
...
assert.equal(definition.loadCase.loadResultant.x, 0);
assert.equal(definition.loadCase.loadResultant.y, -100);
assert.equal(definition.loadCase.expectedReactionMomentAboutFixedEdgeCenter, 10000);
assert.deepEqual(definition.meshLadder.levels.map((row) => row.h), [25, 12.5, 6.25]);
assert.equal(convergencePolicy.frozenRules.refinementRatio, 2);
...
const methods = Object.entries(definition.meshLadder.methods)
  ...
  .map(([method]) => method);
const methodResults = methods.map(runMethod);

const body = {
  schema: 'lafea-b02a-production-qualification-receipt/v1',
...
console.log(JSON.stringify({
  ...
  semanticHash: canonicalLafeaSha256({
    schema: 'lafea-b02a-production-qualification-receipt-hash-input/v1', receipt: body,
  }),
}));
...
function runMethod(method) {
  ...
  const probeAcceptance = finest.probes.map((evidence) => {
    ...
    const limit = evidence.probe.quantityId.startsWith('DISPLACEMENT_')
      ? definition.acceptance.fixedProbeDisplacementRelativeErrorMaximum
      : definition.acceptance.fixedProbeStressRelativeErrorMaximum;
    within(error, limit, `${method}/${frozen.probeId} finest relative error`);
    return Object.freeze({
  ...
  within(
    ...
    `${method} finest strain energy`,
  );

  const convergence = finest.probes.map((finestProbe) => {
    const probeId = finestProbe.probe.probeId;
    const evidenceByLevel = levels.map((row) => row.probes.find((probe) => probe.probe.probeId === probeId));
    ...
    assert.ok(
      ['ASYMPTOTIC', 'MONOTONIC_CONVERGING'].includes(result.classification),
      `${method}/${probeId} convergence is ${result.classification}`,
    );
