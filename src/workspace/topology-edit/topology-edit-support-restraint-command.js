...
export function resolveTopologyEditSupportRestraintTargets(topology, request) {
  ...
  const nodes = support.nodeId
    ...
    : [];
  const hostEdges = resolveHostEdges(topology, support);
  if (hostEdges.length > 1) {
    throw new RangeError(
      `TopologyEditSupportRestraint: support ${support.id} resolves multiple host edges.`,
    );
  }
  return {
    nodes,
    edges: hostEdges.map((edge) => ({
      kind: 'EDGE', role: 'SUPPORT_HOST', id: edge.id,
      revision: semanticHash({ kind: 'EDGE', record: edge }), record: edge,
    })),
    supports: [{
