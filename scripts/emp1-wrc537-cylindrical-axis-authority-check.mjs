...
assert.equal(authority.foundationResultHash, standardResult.semanticHashes.resultPayloadSemanticHash);
assert.deepEqual(authority.frameInput.nozzleCenterlineGlobal, EXPECTED_BASIS.P);
for (const name of COMPONENTS) assert.deepEqual(authority.basisGlobal[name], EXPECTED_BASIS[name], `${name} basis`);
...
for (const name of COMPONENTS) {
  ...
  const positiveInput = isForce ? { forceGlobal: vector, momentGlobal: [0, 0, 0] } : { forceGlobal: [0, 0, 0], momentGlobal: vector };
  const negativeInput = isForce ? { forceGlobal: scale(vector, -1), momentGlobal: [0, 0, 0] } : { forceGlobal: [0, 0, 0], momentGlobal: scale(vector, -1) };
...
const unresolved = deriveEmp1Wrc537CylindricalAxisAuthority({ foundationResult: coincidentResult, foundationModel: coincidentModel, loadCaseIdentity: 'LC-1' });
...
expectCode('coincident-qualified-rejection', () => requireEmp1Wrc537QualifiedCylindricalAxisAuthority(unresolved), 'EMP1_WRC537_CYLINDRICAL_AXIS_AUTHORITY_NOT_QUALIFIED');
...
expectCode('nonradial-source-target', () => deriveEmp1Wrc537CylindricalAxisAuthority({ foundationResult: skewResult, foundationModel: skewModel, loadCaseIdentity: 'LC-1' }), 'EMP1_WRC537_AXIS_SOURCE_TO_TARGET_NOT_RADIAL');

const tampered = structuredClone(authority);
tampered.basisGlobal.Mc = [1, 0, 0];
expectCode('basis-tamper', () => requireEmp1Wrc537QualifiedCylindricalAxisAuthority(tampered), 'EMP1_WRC537_CYLINDRICAL_AXIS_BASIS_DRIFT:Mc');
...
const cauxAuthority = deriveEmp1Wrc537CylindricalAxisAuthority({ foundationResult: cauxResult, foundationModel: cauxModel, loadCaseIdentity: 'LC-1' });
const cauxFrame = buildEmp1Wrc537CylindricalFrame(cauxAuthority.frameInput);
const cauxObserved = emp1GlobalLoadsToWrc537(cauxFrame, { forceGlobal: [-161, -2109, 53], momentGlobal: [775, -121, -33] });
...
console.log(JSON.stringify({
  ...
  failClosedFalsifiers: ['coincident-source-target', 'nonradial-source-target', 'basis-tamper'],
...
function assertOneHot(actual, name, expected, label) {
  for (const component of COMPONENTS) assert.equal(actual[component], component === name ? expected : 0, `${label} ${component}`);
}
...
function expectCode(name, fn, prefix) {
  ...
  assert.ok(String(caught.code ?? caught.message).startsWith(prefix), `${name}: actual=${caught.code ?? caught.message}`);
}
