...
assert.equal(current.currentnessNotice, null,
  'current reportable local result must not show stale/blocked remediation');
...
assert.equal(current.steps.some((step) => /\bCALCULATED\b|\bREADY\b|ROUTE SUSPENDED/u.test(step.statusLabel)), false,
  'primary statuses must not leak backing runtime state');
...
assert.equal(staleInput.authoritySummary.localMethod, 'LOCAL METHOD QUALIFIED');
...
assert.equal(staleInput.steps[5].statusLabel, 'LOCAL METHOD QUALIFIED · LOCAL RESULT STALE');
assert.equal(staleInput.steps[6].statusLabel, 'LOCAL RESULT STALE · RELEASE PROFILE NOT QUALIFIED');
...
assert.deepEqual(staleAuthority.currentnessNotice.reasonCodes,
...
const staleBlocked = buildEmp1ProfessionalWorkflowPresentation(projection({
  ...
  cBlockers: ['EMP1_WORKBENCH_C_CURRENT_ROUTE_NOT_AUTHORIZED'],
}));
...
assert.match(staleBlocked.currentnessNotice.action,
  /Restore or qualify the current bounded WRC route authority/u);
...
assert.match(noC.currentnessNotice.reasons[0], /not production-authorized/u);
...
const unknownBlocked = buildEmp1ProfessionalWorkflowPresentation(projection({
  ...
  cRetainedResultAvailable: false,
  cRunAuthorized: false,
  cBlockers: ['EMP1_WRC_UNMAPPED_BLOCKER'],
}));
...
assert.equal(unknownBlocked.currentnessNotice.reasons[0], 'EMP1 WRC UNMAPPED BLOCKER',
  'unknown governed blocker codes must remain visibly represented');
...
assert.equal(analyticalSource.includes('renderEmp1AssessmentWorkflow(root, projection'), false,
  'internal A/B/C workflow renderer must not remain primary');
...
assert.match(viewSource, /dataset\.role = 'emp1-professional-currentness-notice'/u,
  'primary workflow must expose stale/blocked explanation');
assert.match(viewSource, /item\.dataset\.reasonCode = reasonCode/u,
  'governed reason codes must remain visible as evidence');
assert.match(viewSource, /dataset\.role = 'emp1-professional-required-action'/u,
  'stale/blocked workflow must state required engineer action');
...
assert.equal(presentationSource.includes('../core/emp1/'), false,
  'presentation must not create a parallel core dependency');
assert.equal(presentationSource.includes('runEmp1'), false,
  'presentation must not invoke calculation');
assert.equal(presentationSource.includes('gamma'), false,
  'presentation must not calculate or infer WRC numerical parameters');

console.log(JSON.stringify({
  schema: 'emp1-professional-workflow-check/v3',
  status: 'PASS_WRC_PROFESSIONAL_WORKFLOW_CURRENTNESS_EXPLANATION',
  labels,
  backing,
  ...
  staleReasonEvidencePreserved: true,
  unknownReasonFailsVisible: true,
