...
const ledgerPath='validation/emp1/wrc537-2013/cylindrical-higher-gamma-beta-limit-ledger-v2.json';
...
const historicalGamma5Qualification='3b4375407dc9484c80144f2d9a5b555000d0257021108cd799923ed6fede1a8e';
const routeSuspensionReason='WRC_GAMMA5_ROUTE_REQUALIFICATION_REQUIRED_AFTER_SOURCE_AUTHORITY_CLOSURE';
...
assert.equal(ledger.schema,'emp1-wrc537-cylindrical-higher-gamma-beta-limit-ledger/v2');
...
assert.equal(ledger.historicalGamma5Qualification.gamma,5);
assert.equal(ledger.historicalGamma5Qualification.betaMinimum,0.05);
assert.equal(ledger.historicalGamma5Qualification.betaMaximum,0.5);
assert.equal(
  ledger.historicalGamma5Qualification.status,
  'HISTORICAL_PRE_SOURCE_AUTHORITY_CLOSURE_QUALIFICATION',
);
assert.equal(ledger.historicalGamma5Qualification.routeQualificationSha256,historicalGamma5Qualification);
assert.equal(EMP1_WRC537_GAMMA5_ZERO_DP_ROUTE_QUALIFICATION_SHA256,historicalGamma5Qualification);

const currentRoute=EMP1_C_BOUNDED_PRODUCTION_ROUTES.find(
  (row)=>row.routeId===EMP1_C_WRC537_GAMMA5_ZERO_DP_ROUTE_ID,
);
assert.ok(currentRoute,'current gamma5 bounded-route registry row missing');
assert.equal(EMP1_WRC537_GAMMA5_ZERO_DP_ROUTE_AUTHORIZED,false);
assert.deepEqual(EMP1_WRC537_GAMMA5_ZERO_DP_ROUTE_SUSPENSION_REASONS,[routeSuspensionReason]);
assert.equal(currentRoute.registered,false);
assert.equal(currentRoute.engineeringUseAuthorized,false);
assert.equal(currentRoute.comparisonQualificationAvailable,true);
assert.deepEqual(currentRoute.suspensionReasons,[routeSuspensionReason]);
assert.equal(currentRoute.globalEmp1CRouteAuthority,false);
assert.equal(currentRoute.releaseQualified,false);
assert.equal(ledger.currentGamma5ProductionState.routeAuthorized,false);
assert.equal(ledger.currentGamma5ProductionState.registryRegistered,false);
assert.equal(ledger.currentGamma5ProductionState.engineeringUseAuthorized,false);
assert.equal(ledger.currentGamma5ProductionState.comparisonQualificationAvailable,true);
assert.equal(ledger.currentGamma5ProductionState.suspensionReason,routeSuspensionReason);
assert.equal(ledger.currentGamma5ProductionState.globalEmp1CRouteAuthority,false);
assert.equal(ledger.currentGamma5ProductionState.releaseQualified,false);

assert.equal(ledger.visualReobservation.status,'PASS_SOURCE_CHARTS_REOBSERVED_NO_EXACT_NUMERIC_HIGHER_GAMMA_ENDPOINTS');
assert.equal(
  ledger.visualReobservation.evidenceRole,
...
for(const prohibition of [
  ...
  'DO_NOT_TREAT_HISTORICAL_GAMMA5_QUALIFICATION_AS_CURRENT_PRODUCTION_AUTHORITY',
...
const semanticPayload={
  ...
  historicalGamma5Qualification:ledger.historicalGamma5Qualification,
  currentGamma5ProductionState:ledger.currentGamma5ProductionState,
  higherGammaAssessments:ledger.higherExactTabulatedGammaAssessments,
  prohibitions:ledger.prohibitions,
  visualConclusion:{
    status:ledger.visualReobservation.status,
    evidenceRole:ledger.visualReobservation.evidenceRole,
    digitizationPerformed:false,
  },
};
const result={
  schema:'emp1-wrc537-cylindrical-higher-gamma-beta-limit-check/v3',
  ...
  historicalGamma5QualificationPreserved:true,
  currentGamma5ProductionRouteAuthorized:false,
  currentGamma5RegistryRegistered:false,
