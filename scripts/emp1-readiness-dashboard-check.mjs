...
const current = projectEmp1Readiness(projection({
  cState: 'CALCULATED_CURRENT', cResultAvailable: true, cRetainedResultAvailable: true,
}));
...
const ready = projectEmp1Readiness(projection({
  cState: 'READY_TO_RUN', cResultAvailable: false, cRetainedResultAvailable: false,
}));
...
const stale = projectEmp1Readiness(projection({
  cState: 'STALE_INPUT', cResultAvailable: false, cRetainedResultAvailable: true,
...
const blocked = projectEmp1Readiness(projection({
  cState: 'ROUTE_SUSPENDED', cResultAvailable: false, cRetainedResultAvailable: false,
...
const missing = projectEmp1Readiness(projection({
  cState: 'SOURCE_INCOMPLETE', cResultAvailable: false, cRetainedResultAvailable: false,
  cRunAuthorized: false, aDocumentLoaded: false, bDocumentLoaded: false,
  aResultAvailable: false, bResultAvailable: false,
...
assert.equal((viewSource.match(/projectEmp1Readiness\(/gu) ?? []).length, 1,
...
assert.match(viewSource,
  /does not itself establish method authority, applicability authority, engineering review, code compliance, or release qualification/u);

const expectedLabels = [
  'Source & input', 'Bounded method', 'Applicability', 'Calculation',
  'Engineering review', 'Code compliance', 'Release qualification',
];
...
console.log(JSON.stringify({
  schema: 'emp1-readiness-dashboard-check/v2',
  status: 'PASS_READ_ONLY_CORE_READINESS_WITH_GOVERNED_REVIEW_STATE',
  ...
  reviewEvidenceReevaluatedByUi: false,
...
function projection({
  cState, cResultAvailable, cRetainedResultAvailable, cRunAuthorized = true,
  cBlockers = [], aDocumentLoaded = true, bDocumentLoaded = true,
  aResultAvailable = true, bResultAvailable = true, bRetainedResultAvailable = false,
  aState = null, bState = null, releaseQualified = false,
}) {
  return {
    ...
    qualificationBoundary: {
      passIsCodeCompliance: false, releaseQualified, emp1CProductionAuthority: cRunAuthorized,
    },
    steps: [
      {
        ...
        documentLoaded: aDocumentLoaded, resultAvailable: aResultAvailable,
        retainedResultAvailable: aResultAvailable, runAuthorized: true,
      },
      {
      ...
      },
      {
        ...
        state: cState, documentLoaded: aDocumentLoaded && bDocumentLoaded,
        resultAvailable: cResultAvailable, retainedResultAvailable: cRetainedResultAvailable,
        runAuthorized: cRunAuthorized, blockers: Object.freeze([...cBlockers]),
      },
    ],
  };
}
