...
const currentPlanPath = path.join(repoRoot, 'validation/lafea-independent-qualification/plan-v1.json');
const currentRunnerPath = path.join(scriptDir, 'lafea-independent-qualification.mjs');
const currentVerifierPath = path.join(scriptDir, 'lafea-independent-qualification-verify.mjs');
...
const headProbe = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: repoRoot, encoding: 'utf8' });
assert.equal(headProbe.status, 0, headProbe.stderr);
const expectedHead = headProbe.stdout.trim();
assert.match(expectedHead, /^[0-9a-f]{40}$/u);
const wrongHead = expectedHead === '0'.repeat(40) ? '1'.repeat(40) : '0'.repeat(40);
const currentPlanText = fs.readFileSync(currentPlanPath, 'utf8');
const currentPlan = JSON.parse(currentPlanText);
assert.equal(
  `${JSON.stringify(currentPlan, null, 2)}\n`,
  currentPlanText,
  'TECH-12D fixture requires canonical pretty plan bytes to match checked-in plan.',
);
const currentRunnerSha256 = fileSha256(currentRunnerPath);
const currentVerifierSha256 = fileSha256(currentVerifierPath);
const currentPackageLockSha256 = fileSha256(packageLockPath);
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lafea-tech12d-'));
const rows = [];

try {
  // Fully PASS, exact-head, integrity-valid fixture using the actual checked-in
  // plan, runner, verifier and package-lock identities. This authorizes only a
  // future promotion change; TECH-12D itself still has zero product effect.
  ...
  assert.equal(passRecord.planSha256, fileSha256(currentPlanPath));
  assert.equal(passRecord.runnerSha256, currentRunnerSha256);
  assert.equal(passRecord.packageLockSha256, currentPackageLockSha256);
  assert.equal(passRecord.verifierSha256, currentVerifierSha256);
  ...
  // The bundle may itself be internally exact-head consistent yet be for a
  // different commit. Running the generator from this exact checkout must block
  // the substitution instead of consuming the older/newer bundle.
  ...
  rows.push({ case: 'WRONG_BUNDLE_HEAD', status: wrongHeadRecord.status });

  // Browser skip cannot authorize promotion even if all engineering command
  // rows were otherwise marked PASS.
  ...
  // still be an internally self-consistent evidence bundle. TECH-12D rejects it
  // both for required-step loss and because the plan no longer matches this
  // exact checkout's qualified plan.
  ...
  assert.equal(missingRecord.toolingParityVerified, false);
  ...
  assert.equal(standaloneRecord.productionBindingAuthorized, false);
  rows.push({ case: 'STANDALONE_MATH_ONLY', status: standaloneRecord.status });

  // Bundle integrity can remain valid while the copied verifier differs from
  // the exact checkout. Tooling parity must independently block that case.
  const verifierMismatchBundle = makeBundle(
    'verifier-mismatch', expectedHead, null, { mutateBundledVerifier: true },
  );
  const verifierMismatch = runGenerator(
    verifierMismatchBundle, expectedHead, 'verifier-mismatch-record.json',
  );
  assert.equal(verifierMismatch.status, 2, verifierMismatch.stderr);
  const verifierMismatchRecord = readRecord('verifier-mismatch-record.json');
  assert.equal(verifierMismatchRecord.toolingParityVerified, false);
  assert.ok(verifierMismatchRecord.reasons.includes('ACTIVATION_TOOLING_PARITY_NOT_VERIFIED'));
  rows.push({ case: 'VERIFIER_PARITY_MISMATCH', status: verifierMismatchRecord.status });
...
function makeBundle(name, head, mutate = null, options = {}) {
  const bundle = path.join(tempRoot, name);
  const toolsDir = path.join(bundle, 'tools');
  ...
  const manifest = {
    ...
    packageLockSha256: currentPackageLockSha256,
  };
  ...
  fs.copyFileSync(
    currentRunnerPath,
    path.join(toolsDir, 'lafea-independent-qualification.mjs'),
  );
  fs.copyFileSync(
    currentVerifierPath,
    path.join(toolsDir, 'lafea-independent-qualification-verify.mjs'),
  );
