...
export const LFEA_PUBLICATION_STATUS = Object.freeze({ READY: 'READY', BLOCKED: 'BLOCKED' });
...
function b31Readiness(preFlight, resultsState, authority) {
  ...
  if (!currentCompilation(preFlight)) reasons.push('CURRENT_MECHANICAL_COMPILATION_REQUIRED');
  if (!currentRecovery(resultsState)) reasons.push('CURRENT_B3_4_RECOVERY_REQUIRED');
  if (!authority?.codeRecoveryByCase) reasons.push('COMPONENT_CODE_POINT_RECOVERY_REQUIRED');
...
function publicationStage(stage, producerChain, reasons) {
  ...
  return deepFreeze({
    ...
    status: uniqueReasons.length === 0
      ? LFEA_PUBLICATION_STATUS.READY : LFEA_PUBLICATION_STATUS.BLOCKED,
