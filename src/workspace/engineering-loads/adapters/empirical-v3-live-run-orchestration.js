...
export function executeEmpiricalV3LiveSourceBoundRun(input) {
  ...
  const started = event({
    ...
    messageCode: 'EMP_V3_CALC_STARTED',
  ...
  try {
    ...
    error.empiricalV3EngineeringEvent = event({
      ...
      riskRefs: riskRefs(packageValue),
      confirmationRefs: confirmationRefs(packageValue),
      messageCode: 'EMP_V3_CALC_BLOCKED',
      messageParameters: { code: error?.code ?? 'EXECUTION_REJECTED' },
      auditMetadata: input?.auditMetadata,
    });
    throw error;
  }
  ...
  if (workflow.state !== 'RESULT_REVIEW_REQUIRED') {
    throw new Error(`Live V3 run projected unexpected workflow ${workflow.state}.`);
  }
  const completed = event({
    ...
    messageCode: 'EMP_V3_CALC_COMPLETED',
    auditMetadata: input?.auditMetadata,
  });
...
export function applyEmpiricalV3LiveResultReview(input) {
  ...
  const reviewed = event({
    ...
    runId: packageValue.runId,
    fromWorkflowState: packageValue.workflow.state,
    toWorkflowState: workflow.state,
    authorityRefs: [authorizationRef(packageValue.calculationAuthorization)],
    calculationEvidenceRefs: [evidenceRef(evidence)],
    messageCode: 'EMP_V3_RESULT_REVIEWED',
    auditMetadata: input?.auditMetadata,
  });
  const nextPackage = resealPackage(packageValue, workflow, [
    record('RESULT_REVIEW', review.receiptId, review),
    record('ENGINEERING_EVENT', reviewed.eventId, reviewed),
  ]);
  return Object.freeze({ resultReview: review, event: reviewed, nextPackage });
}
...
export function applyEmpiricalV3LiveAuditReadiness(input) {
  ...
  const ready = event({
    eventType: 'AUDIT_READY',
    runId: packageValue.runId,
    fromWorkflowState: packageValue.workflow.state,
    toWorkflowState: workflow.state,
    calculationEvidenceRefs: [evidenceRef(evidence)],
    messageCode: 'EMP_V3_AUDIT_READY',
    auditMetadata: input?.auditMetadata,
  });
  const nextPackage = resealPackage(packageValue, workflow, [
    record('AUDIT_READINESS', readiness.readinessId, readiness),
    record('ENGINEERING_EVENT', ready.eventId, ready),
  ]);
  return Object.freeze({ auditReadiness: readiness, event: ready, nextPackage });
}
...
export function reconcileEmpiricalV3LiveAuthorization(input) {
  ...
  const invalidated = event({
    eventType: 'CALC_AUTHORIZATION_INVALIDATED',
    severity: 'ERROR',
    runId: packageValue.runId,
    fromWorkflowState: packageValue.workflow.state,
    toWorkflowState: workflow.state,
    authorityRefs: [authorizationRef(authorization)],
    messageCode: 'EMP_V3_CALC_AUTHORIZATION_INVALIDATED',
    messageParameters: { reasons: assessment.reasons },
    auditMetadata: input?.auditMetadata,
  });
  return Object.freeze({
    ...assessment,
    event: invalidated,
    nextPackage: resealPackage(packageValue, workflow, [record('ENGINEERING_EVENT', invalidated.eventId, invalidated)]),
  });
}
...
export function createEmpiricalV3LiveAuditExport(input) {
  ...
  return createEmpiricalV3AuditJsonExport({
    safetyPackage: packageValue,
    evidence: input?.evidence,
    resultReview: input?.resultReview,
...
function event(input) {
  return sealEmpiricalV3EngineeringEvent({
    ...
    branchId: null, entityIds: [], quantityIds: [], authorityRefs: [], riskRefs: [], confirmationRefs: [], calculationEvidenceRefs: [], messageParameters: {},
    ...input,
  });
}
...
function requireEvidenceMatchesPackage(value, packageValue) {
  if (!value || value.runId !== packageValue.runId || value.authorizationRef?.semanticHash !== packageValue.calculationAuthorization?.semanticHash) {
    throw new Error('Calculation evidence is stale or belongs to another package authorization.');
  }
  return value;
}
