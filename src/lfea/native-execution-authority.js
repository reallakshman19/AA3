...
export const LFEA_NATIVE_EXECUTION_CURRENTNESS = Object.freeze({ NONE: 'NONE', CURRENT: 'CURRENT', STALE: 'STALE' });

/** Own current/stale application state separately from immutable solver evidence. */
export function createLfeaNativeExecutionAuthority(options = {}) {
  const executeAuthorizedCases = options.executeAuthorizedCases ?? executeInputXmlAuthorizedRawCases;
  if (typeof executeAuthorizedCases !== 'function') throw new TypeError('LFEA native execution requires an executor function.');
  ...
  function run(preFlightRecord, options = {}) {
    ...
    if (!preFlight.solveAuthorized || preFlight.authorization === null) {
      throw authorityError('LFEA_NATIVE_EXECUTION_AUTHORIZATION_REQUIRED',
        'A sealed current native InputXML solve authorization is required before execution.');
    }
    const requestedCaseIds = normalizeRequestedCases(options.requestedCaseIds
      ?? preFlight.authorization.authorizedPhysicalCaseIds);
    const execution = solveInputXmlLinearAnalysis(preFlight.preparation, preFlight.authorization, {
      requestedCaseIds,
  ...
  function reconcile(preFlightRecord) {
    ...
    catch { state = staleState(state, ['CURRENT_PREFLIGHT_INVALID']); return state; }
    ...
    if (!preFlight.solveAuthorized || preFlight.authorization === null) reasons.push('AUTHORIZATION_NO_LONGER_CURRENT');
    else {
      const allowed = new Set(preFlight.authorization.authorizedPhysicalCaseIds);
      if (state.execution.requestedCaseIds.some((caseId) => !allowed.has(caseId))) reasons.push('EXECUTED_CASE_NO_LONGER_AUTHORIZED');
    }
    state = reasons.length > 0 ? staleState(state, reasons) : Object.freeze({
      currentness: LFEA_NATIVE_EXECUTION_CURRENTNESS.CURRENT,
  ...
  return Object.freeze({
    run, reconcile, clearCurrentAuthority,
    getState: () => state,
    getCurrentExecution: () => state.currentness === LFEA_NATIVE_EXECUTION_CURRENTNESS.CURRENT ? state.execution : null,
    getCurrentQualifiedExecution: () => state.currentness === LFEA_NATIVE_EXECUTION_CURRENTNESS.CURRENT
      && ['QUALIFIED', 'CONDITIONAL'].includes(state.execution?.status) ? state.execution : null,
  });
}

function emptyState() { return Object.freeze({ currentness: 'NONE', execution: null, staleReasonCodes: Object.freeze([]), parent: null }); }
function staleState(previous, reasons) { return Object.freeze({
  currentness: 'STALE', execution: previous.execution,
  staleReasonCodes: Object.freeze([...new Set(reasons)].sort(compareAscii)), parent: previous.parent,
}); }
function parentIdentity(preFlight) { return Object.freeze({
...
function normalizeRequestedCases(value) {
  if (!Array.isArray(value) || value.length === 0 || value.some((id) => typeof id !== 'string' || id.trim() === '')) {
    throw authorityError('LFEA_NATIVE_EXECUTION_CASE_SET_INVALID', 'Native execution requires a non-empty requested physical-case set.');
  }
  return Object.freeze([...new Set(value.map((id) => id.trim()))].sort(compareAscii));
}
...
function authorityError(code, message) { const error = new TypeError(message); error.code = code; error.analysisStage = 'LFEA_NATIVE_EXECUTION_AUTHORITY'; return error; }
