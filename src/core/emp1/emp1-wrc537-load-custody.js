...
const RUNTIME_SEMANTIC_HASH=/^(?:fnv1a64:[a-f0-9]{16}|[a-f0-9]{64})$/u;
...
export const EMP1_WRC537_LOAD_CUSTODY_STATUSES=Object.freeze(['PASS_QUALIFIED_UPSTREAM_LOAD_PACKAGE','PASS_COMPARISON_FIXTURE_ONLY']);
export const EMP1_WRC537_PRESSURE_MODES=Object.freeze(['SOURCE_LOAD_ALREADY_INCLUDES_THRUST','SOURCE_LOAD_EXCLUDES_THRUST_NOT_REQUIRED','SOURCE_LOAD_EXCLUDES_THRUST_ADDED_UPSTREAM']);
...
export function normalizeEmp1Wrc537LoadCustody(value){
  ...
  const sourceLoadCustodyHash=runtimeHash(value.sourceLoadCustodyHash,'SOURCE_LOAD_CUSTODY_HASH');
  const loadPackageSemanticHash=runtimeHash(value.loadPackageSemanticHash,'LOAD_PACKAGE_SEMANTIC_HASH');
  ...
  return deepFreeze({schema:EMP1_WRC537_LOAD_CUSTODY_SCHEMA,status:value.status,engineeringUseAuthorized:value.engineeringUseAuthorized,productionRouteInputAuthorized:false,sourceId,loadCaseId,loadReference:value.loadReference,pressureThrustDisposition:value.pressureThrustDisposition,sourceLoadCustodyHash,loadPackageSemanticHash,productionObservationUsedToSetAuthority:false,producerQualification:producer,pressureThrust:pressure});
}
...
export function requireEmp1Wrc537QualifiedLoadCustody(value,{expectedProducerQualificationHash}={}){
  ...
  const expected=authorityHash(expectedProducerQualificationHash,'EXPECTED_PRODUCER_QUALIFICATION_HASH');
...
function normalizeProducer(value){
  ...
  const qualificationRecordHash=authorityHash(value.qualificationRecordHash,'PRODUCER_QUALIFICATION_HASH');
  ...
  return deepFreeze({producerId,authorityClass:value.authorityClass,status:value.status,qualificationRecordHash,productionObservationUsedToSetAuthority:false});
}
function normalizePressure(value){
  ...
  const policyRecordHash=authorityHash(value.policyRecordHash,'PRESSURE_POLICY_HASH');
  ...
  return deepFreeze({status:value.status,mode:value.mode,doubleCountGuardQualified:true,sourceLoadContainsPressureThrust:value.sourceLoadContainsPressureThrust,upstreamAdditionVerified:value.mode==='SOURCE_LOAD_EXCLUDES_THRUST_ADDED_UPSTREAM'?true:null,policyRecordHash,productionObservationUsedToSetAuthority:false});
}
function authorityHash(value,label){if(typeof value!=='string'||!SHA256_HEX.test(value)) throw custodyError(`EMP1_WRC537_LOAD_CUSTODY_${label}_INVALID`);return value;}
function runtimeHash(value,label){if(typeof value!=='string'||!RUNTIME_SEMANTIC_HASH.test(value)) throw custodyError(`EMP1_WRC537_LOAD_CUSTODY_${label}_INVALID`);return value;}
