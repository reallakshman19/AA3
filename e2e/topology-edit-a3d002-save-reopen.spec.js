...
test('A3D-002 visible edit survives exact Undo Redo Save destroy remount Reload', async ({ page }, testInfo) => {
  ...
  const draftAuditPanel = host.locator('details[data-panel-kind="draft"]');
  await expect(draftAuditPanel).toBeAttached();
  await expect(draftAuditPanel).not.toHaveAttribute('open', '');
  await draftAuditPanel.locator(':scope > summary').click();
  await expect(draftAuditPanel).toHaveAttribute('open', '');
  const reloadButton = draftAuditPanel.getByRole('button', { name: 'Reload draft', exact: true });
  ...
  const evidence = {
    schema: 'TopologyEditA3D002SaveReopenEvidence.v4',
    status: 'PASS',
    ...
    visibleFlow: [
      ...
      'REENTER_3D_EDIT', 'VERIFY_DISTINCT_CONTROLLER', 'OPEN_DRAFT_AUDIT',
      'RELOAD_DRAFT', 'REOPEN_TABLE_AND_VERIFY_S-007',
    ],
  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      webglCanvasCount: document.querySelectorAll(
        'canvas[data-viewport-backend="topology-edit-webgl"]',
      ).length,
...
async function remountCustodyEvidence(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      controllerIsDistinctFromPreExit: Boolean(mounted)
        && mounted !== globalThis.__a3d002PreExitController,
      webglCanvasCount: document.querySelectorAll(
        'canvas[data-viewport-backend="topology-edit-webgl"]',
      ).length,
    };
