...
export function validateEmp1Source(value) {
  ...
  if (source.schema !== EMP1_SCHEMAS.SOURCE || source.productId !== EMP1_PRODUCT_ID) {
    throw emp1SourceError('EMP1_SOURCE_IDENTITY_INVALID');
  }
  return source;
}
...
function normalizeLocalMethod(value) {
  if (value == null) {
    return { requested: false, qualificationState: 'NOT_REQUESTED', sourceSha256: null };
  }
  requireRecord(value, 'localMethod');
  const result = structuredClone(value);
  result.requested = Boolean(result.requested);
  if (result.sourceSha256 != null && !SHA256_HEX.test(result.sourceSha256)) {
    throw emp1SourceError('EMP1_LOCAL_METHOD_SOURCE_SHA256_INVALID');
  }
  return result;
}
...
function normalizeBenchmark(value) {
  ...
  if (value.sourceSha256 != null && !SHA256_HEX.test(value.sourceSha256)) {
    throw emp1SourceError('EMP1_BENCHMARK_SOURCE_SHA256_INVALID');
  }
  return structuredClone(value);
}

function requireRecord(value, path) {
  if (!isRecord(value)) throw emp1SourceError(`EMP1_SOURCE_FIELD_INVALID:${path}`);
}

function requireArray(value, path) {
  if (!Array.isArray(value)) throw emp1SourceError(`EMP1_SOURCE_FIELD_INVALID:${path}`);
}

function requireString(value, path) {
  if (typeof value !== 'string' || !value.trim()) {
    throw emp1SourceError(`EMP1_SOURCE_FIELD_INVALID:${path}`);
  }
}
