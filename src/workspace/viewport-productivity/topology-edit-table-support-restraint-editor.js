...
export function renderTopologyEditTableSupportRestraintEditor(row, stagedIntent) {
  ...
  return `<section class="topology-edit-table__editor" data-table-support-restraint-editor="${esc(row.identity.canonicalId)}">
    ...
    <p class="topology-edit-table__notice">Stage one explicit canonical restraint override. Imported restraint evidence is retained. Host and station remain read-only until support placement is separately certified.</p>
