...
export const EMP1_WRC537_CYL_TABLE5_SCHEMA = 'emp1-wrc537-cylindrical-table5-result/v1';
export const EMP1_WRC537_CYL_LOCATIONS = LOCATIONS;

/**
 * Reconstruct WRC537 Table 5 cylindrical-shell local stresses from already
 * qualified nondimensional curve ordinates. Gamma/beta curve selection is out
 * of scope here; beta is used only in the Table 5 dimensional load scales.
 *
 * Units must be consistent: if force=N and length=mm, returned stresses are
 * N/mm^2. Moments must be force*length in that same length unit.
 */
export function evaluateEmp1Wrc537CylindricalTable5(input) {
  ...
  const stressIntensity = LOCATIONS.map((_, index) => planeStressTresca(
    circumferential[index], longitudinal[index], shear[index],
  ));
  ...
  return deepFreeze({
    ...
    scale,
    components: {
      circumferential: circumferentialComponents,
      longitudinal: longitudinalComponents,
      shear: shearComponents,
    },
    stresses: { circumferential, longitudinal, shear, stressIntensity },
  });
}

export function emp1Wrc537PlaneStressIntensity(sigmaPhi, sigmaX, tau) {
  return planeStressTresca(
    finite(sigmaPhi, 'SIGMA_PHI'),
    finite(sigmaX, 'SIGMA_X'),
    finite(tau, 'TAU'),
  );
}

function planeStressTresca(sigmaPhi, sigmaX, tau) {
...
function normalizeLoads(value = {}) {
  return deepFreeze({
    ...
    Vc: finite(value.Vc, 'LOAD_VC'),
    Vl: finite(value.Vl, 'LOAD_VL'),
    Mc: finite(value.Mc, 'LOAD_MC'),
    Ml: finite(value.Ml, 'LOAD_ML'),
    Mt: finite(value.Mt, 'LOAD_MT'),
  });
}
function normalizeOrdinates(value = {}) {
  return deepFreeze({
    circ: normalizeFamily(value.circ, 'CIRC'),
    long: normalizeFamily(value.long, 'LONG'),
  });
}
function normalizeFamily(value, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw table5Error(`EMP1_WRC537_TABLE5_${label}_ORDINATES_REQUIRED`);
  const keys = ['Pmem_AB','Pmem_CD','Pbend_AB','Pbend_CD','Mcmem','Mcbend','Mlmem','Mlbend'];
  return Object.fromEntries(keys.map((key) => {
    const number = finite(value[key], `${label}_${key.toUpperCase()}`);
    if (number < 0) throw table5Error(`EMP1_WRC537_TABLE5_${label}_${key.toUpperCase()}_NEGATIVE_ORDINATE`);
    return [key, number];
  }));
}
function applySigns(signs, load, magnitude) {
  const values = Array.isArray(magnitude) ? magnitude : Array(signs.length).fill(magnitude);
  const direction = load < 0 ? -1 : 1;
  return signs.map((sourceSign, index) => normalizeZero(sourceSign * direction * values[index]));
}
function grouped(ab, cd) { return [ab,ab,ab,ab,cd,cd,cd,cd]; }
function sumComponentArrays(components) {
  const rows = Object.values(components);
  return LOCATIONS.map((_, index) => normalizeZero(rows.reduce((sum, row) => sum + row[index], 0)));
}
function positive(value, label) {
  const number = finite(value, label);
  if (number <= 0) throw table5Error(`EMP1_WRC537_TABLE5_${label}_NONPOSITIVE`);
  return number;
}
function finite(value, label) {
  if (!Number.isFinite(value)) throw table5Error(`EMP1_WRC537_TABLE5_${label}_INVALID`);
  return value;
}
