...
export async function captureA3d004Authority(page) {
  return page.evaluate((collections) => {
    ...
    if (!controller || !topology || !journal) throw new Error('A3D-004: mounted production authority is unavailable.');
    return {
    ...
    function recordSnapshot(value, names) {
      const records = {};
      for (const collection of names) for (const record of value[collection] ?? []) records[`${collection}:${record.id}`] = JSON.stringify(record);
      return records;
    }
  }, COLLECTIONS);
}
...
export async function captureA3d004Preview(page, surface = 'authoring') {
  return page.evaluate(({ surfaceId, collections }) => {
    ...
    const journal = controller?.session?.journal;
    if (!controller || !current || !journal) throw new Error('A3D-004: mounted production authority is unavailable.');
    const candidate = surfaceId === 'table' ? controller.tableAdapter?.runtime?.preview?.candidate : controller.authoringRuntime?.candidate;
    if (!candidate?.canonicalTopology) throw new Error(`A3D-004: ${surfaceId} candidate is unavailable after visible Preview.`);
    ...
    return {
      ...
      priorCanonicalHash: candidate.priorCanonicalHash ?? candidate.priorCanonicalTopologyHash ?? '',
      resultingCanonicalHash: candidate.resultingCanonicalHash ?? candidate.canonicalTopologyHash ?? candidate.canonicalTopology.canonicalTopologyHash,
      changedCanonicalIds,
      commandTypes: (candidate.materializedCommandIntents ?? []).map((row) => row.commandType).filter(Boolean),
      projectionSignature: normalizedProjection(projection, accepted),
      ghostSignature: normalizedRenderedGroups([controller.viewportBackend?.groups?.ghostGroup]),
      candidateRecords: recordSnapshot(candidate.canonicalTopology, collections),
    };
    function round(value) { return Number.isFinite(Number(value)) ? Number(Number(value).toFixed(9)) : null; }
    function point(value) { return value && [value.x, value.y, value.z].every(Number.isFinite) ? [round(value.x), round(value.y), round(value.z)] : null; }
    function pick(value = {}, fallbackPartRole = '') {
      return {
        modelRole: String(value.modelRole ?? ''), objectKind: String(value.objectKind ?? ''), objectId: String(value.objectId ?? ''), nodeId: String(value.nodeId ?? ''),
        partRole: String(value.partRole || fallbackPartRole || ''), supportId: String(value.supportId ?? ''), restraintId: String(value.restraintId ?? ''), restraintFamily: String(value.restraintFamily ?? ''),
      };
    }
    function projectionRows(value, plural, compactPlural) { const compact = value?.[compactPlural]; return Array.isArray(compact) ? compact : (value?.[plural] ?? []); }
    function normalizedProjection(value, filter) {
      const rows = [];
      for (const row of projectionRows(value, 'elements', 'compactElements')) if (filter(row)) rows.push({
        kind: 'ELEMENT', id: String(row.entityId ?? row.id ?? ''), type: String(row.type ?? ''), position: [round(row.x), round(row.y), round(row.z)], sizeMm: round(row.sizeMm), pick: pick(row.pickTarget),
      });
      for (const row of projectionRows(value, 'segments', 'compactSegments')) if (filter(row)) rows.push({
        kind: 'SEGMENT', id: String(row.entityId ?? row.id ?? ''), type: String(row.type ?? ''), start: point(row.start), end: point(row.end),
        points: Array.isArray(row.points) ? row.points.map(point) : [], radiusMm: round(row.radiusMm), endRadiusMm: round(row.endRadiusMm), pick: pick(row.pickTarget),
      });
    ...
    function normalizedRenderedGroups(groups) {
      ...
      for (const group of groups.filter(Boolean)) {
        ...
        group.traverse((object) => {
          ...
          const picks = directPick ? [directPick] : (Array.isArray(object.userData?.pickTable) ? object.userData.pickTable : []);
          if (!picks.length) return;
          object.geometry.computeBoundingBox?.();
          const bounds = object.geometry.boundingBox;
          const geometryType = String(object.geometry?.type ?? '');
          const shape = {
            geometryType, position: point(object.position),
            ...
            geometryBounds: geometryType === 'SphereGeometry' || !bounds ? null : { min: point(bounds.min), max: point(bounds.max) },
          };
          const fallbackPartRole = object.userData?.partRole ?? '';
          for (const target of picks) result.push({ pick: pick(target, fallbackPartRole), shape });
        });
      }
...
export async function captureA3d004Applied(page, preview) {
  return page.evaluate(({ changedIds, ghostPicks, collections }) => {
    ...
    const topology = controller?.session?.currentTopology?.();
    const journal = controller?.session?.journal;
    if (!controller || !topology || !journal) throw new Error('A3D-004: applied production authority is unavailable.');
  ...
  }, { changedIds: preview.changedCanonicalIds, ghostPicks: preview.ghostSignature.map((row) => row.pick), collections: COLLECTIONS });
}
...
export function assertA3d004AppliedDifferential(baseline, preview, applied) {
  ...
  expect(applied.activeCommandCount).toBe(baseline.activeCommandCount + preview.commandTypes.length);
  ...
  expect(candidateChangedIds.filter((id) => !declaredChangedIds.has(id)), 'Candidate record changes escaped declared changedCanonicalIds.').toEqual([]);
  ...
  for (const ghostRow of preview.ghostSignature) {
    ...
    expect(shapes.some((shape) => JSON.stringify(shape) === JSON.stringify(ghostRow.shape)), `Applied renderer geometry differs for ghost pick ${key}.`).toBe(true);
  }
}
...
function recordKeyId(key) { return String(key).slice(String(key).indexOf(':') + 1); }
