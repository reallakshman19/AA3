...
// B02D-V2 supersedes only the mesh policy, not the benchmark's method
// applicability, so the registered B02D matrix row remains the authority and
// the V2 frozen definition must agree with it rather than restate it.
...
assert.deepEqual(definition.globalResponseLadder.methods, {
  T3: methodRow.T3, T6: methodRow.T6, Q8: methodRow.Q8,
});
...
const body = {
  schema: 'lafea-b02d-v2-production-qualification-receipt/v1',
  caseId: 'B02D-V2',
  status: 'PASS',
...
console.log(JSON.stringify({
  ...
  semanticHash: canonicalLafeaSha256({
    schema: 'lafea-b02d-v2-production-qualification-receipt-hash-input/v1', receipt: body,
  }),
}, null, 2));
...
function runMethod(method) {
  ...
  const levels = definition.globalResponseLadder.levels.map((level) => {
    ...
    within(loadResultantRelativeError, A.loadResultantRelativeMaximum,
      `B02D-V2/${method}/${level.levelId} load resultant`);
    within(loadMomentRelativeError, A.loadMomentRelativeMaximum,
      `B02D-V2/${method}/${level.levelId} load moment`);
    ...
    within(equilibrium.totalForceRelativeResidual, A.forceEquilibriumRelativeMaximum,
      `B02D-V2/${method}/${level.levelId} force equilibrium`);
    within(equilibrium.totalMomentRelativeResidual, A.momentEquilibriumRelativeMaximum,
      `B02D-V2/${method}/${level.levelId} moment equilibrium`);
...
function probeConvergence(method, probeId, singularityClass, levels, selector, releaseCritical) {
  ...
  assert.equal(evidence.every(Boolean), true, `B02D-V2/${method}/${probeId} evidence missing`);
  const definitionValue = createLafeaContinuumProbeConvergenceDefinition({
    ...
    studyId: `B02D-V2/${method}/${probeId}`,
  ...
  if (releaseCritical) {
    assertConverged({ ...result, gciFineRelative }, `B02D-V2/${method}/${probeId}`);
    ...
    within(gciFineRelative, limit, `B02D-V2/${method}/${probeId} stress GCI`);
  }
  return Object.freeze({
    probeId,
