...
export const EMP1_A_WRC_ZERO_DP_PRODUCER_ROUTE_AUTHORIZED=true;
export const EMP1_A_WRC_ZERO_DP_CANDIDATE_SCHEMA='emp1-a-zero-dp-wrc-load-package-candidate/v1';

/**
 * Convert an already-qualified retained LAFEA.1 result into a WRC-reference
 * load-package candidate, only when differential pressure is exactly zero.
 * The candidate carries no route authority until create...QualifiedLoadCustody
 * binds it to the independently reobserved producer qualification SHA-256.
 */
export function deriveEmp1AZeroDpWrcLoadPackageCandidate({result,loadCaseIdentity,pressureResultIdentity,wrcReferencePointGlobal,productionObservationUsedToSetAuthority=false}={}){
  ...
  verifyAccounting(accounting);verifyLoadResiduals(load);
  const semanticPayload={
    producerId:EMP1_A_WRC_ZERO_DP_PRODUCER_ID,authorityClass:EMP1_A_WRC_ZERO_DP_PRODUCER_AUTHORITY_CLASS,
    upstreamResultPayloadSemanticHash:hashes.resultPayloadSemanticHash,upstreamExecutionEvidenceHash:hashes.executionEvidenceHash,upstreamQualificationEvidenceHash:hashes.qualificationEvidenceHash,
    loadCaseIdentity:load.identity,pressureResultIdentity:pressure.identity,wrcReferencePointGlobal:reference,targetPointGlobal:[...load.targetPointGlobal],referenceResidual,lengthTolerance,
    forceGlobal:[...load.transformedForceGlobal],momentGlobal:[...load.transformedMomentGlobal],forceResidualGlobal:[...load.forceResidualGlobal],momentResidualGlobal:[...load.momentResidualGlobal],commonOriginMomentResidualGlobal:[...load.commonOriginMomentResidualGlobal],
    internalPressure:pressure.internalPressure,externalPressure:pressure.externalPressure,differentialPressure:0,pressureThrust:0,
    pressureMode:'SOURCE_LOAD_EXCLUDES_THRUST_NOT_REQUIRED',loadReference:'WRC_ATTACHMENT_REFERENCE_POINT',productionObservationUsedToSetAuthority:false,
  };
  return deepFreeze({
    schema:EMP1_A_WRC_ZERO_DP_CANDIDATE_SCHEMA,status:'PASS_ZERO_DP_WRC_LOAD_PACKAGE_CANDIDATE',engineeringMechanicsQualified:true,productionRouteInputAuthorized:false,
    producerQualificationSha256:EMP1_A_WRC_ZERO_DP_PRODUCER_QUALIFICATION_SHA256,semanticPayload,
    loadPackageSemanticHash:semanticHash(semanticPayload),sourceLoadCustodyHash:hashes.executionEvidenceHash,
    loadsAtWrcReference:{forceGlobal:[...load.transformedForceGlobal],momentGlobal:[...load.transformedMomentGlobal]},
  });
}
...
export function createEmp1AZeroDpWrcQualifiedLoadCustody(candidate){
  ...
  return deepFreeze({
    ...
    sourceId:candidate.semanticPayload.upstreamResultPayloadSemanticHash,loadCaseId:candidate.semanticPayload.loadCaseIdentity,
    loadReference:'WRC_ATTACHMENT_REFERENCE_POINT',pressureThrustDisposition:'PRESSURE_THRUST_RESOLVED_UPSTREAM',
    sourceLoadCustodyHash:candidate.sourceLoadCustodyHash,loadPackageSemanticHash:candidate.loadPackageSemanticHash,productionObservationUsedToSetAuthority:false,
    producerQualification:{producerId:EMP1_A_WRC_ZERO_DP_PRODUCER_ID,authorityClass:EMP1_A_WRC_ZERO_DP_PRODUCER_AUTHORITY_CLASS,status:'PASS_QUALIFIED_WRC_REFERENCE_LOAD_PRODUCER',qualificationRecordHash:EMP1_A_WRC_ZERO_DP_PRODUCER_QUALIFICATION_SHA256,productionObservationUsedToSetAuthority:false},
    pressureThrust:{status:'PASS_RESOLVED_UPSTREAM',mode:'SOURCE_LOAD_EXCLUDES_THRUST_NOT_REQUIRED',doubleCountGuardQualified:true,sourceLoadContainsPressureThrust:false,policyRecordHash:EMP1_A_WRC_ZERO_DP_PRODUCER_QUALIFICATION_SHA256,productionObservationUsedToSetAuthority:false},
  });
}
...
function verifyResultHashes(result){
  ...
  const actual=reconstructResultHashes(result);for(const [key,value] of Object.entries(actual)) if(result.semanticHashes[key]!==value) throw producerError(`EMP1_A_WRC_ZERO_DP_RESULT_HASH_DRIFT:${key}`);return actual;
}
function verifyAccounting(accounting){
  ...
  for(const [key,toleranceKey] of [['forceResidualGlobal','force'],['momentResidualGlobal','moment'],['commonOriginMomentResidualGlobal','commonOriginMoment']]){
    const residual=vector3(accounting[key],`ACCOUNTING_${key}`),tolerance=accounting.tolerances?.[toleranceKey];
...
function verifyLoadResiduals(load){
  for(const [key,toleranceKey] of [['forceResidualGlobal','force'],['momentResidualGlobal','moment'],['commonOriginMomentResidualGlobal','commonOriginMoment']]){
    const residual=vector3(load[key],`LOAD_${key}`),tolerance=load.tolerances?.[toleranceKey];
...
function selectUnique(rows,identity,label,key='identity'){if(typeof identity!=='string'||!identity.trim()) throw producerError(`EMP1_A_WRC_ZERO_DP_${label}_IDENTITY_REQUIRED`);const matches=rows.filter((row)=>row?.[key]===identity);if(matches.length!==1) throw producerError(`EMP1_A_WRC_ZERO_DP_${label}_NOT_UNIQUE`);return matches[0];}
function toleranceFor(profile,quantity,...values){const rule=profile?.tolerances?.[quantity];if(!record(rule)||!Number.isFinite(rule.absolute)||!Number.isFinite(rule.relative)||rule.absolute<0||rule.relative<0) throw producerError(`EMP1_A_WRC_ZERO_DP_TOLERANCE_PROFILE:${quantity}`);return rule.absolute+rule.relative*Math.max(1,...values.map((value)=>Math.abs(value)));}
