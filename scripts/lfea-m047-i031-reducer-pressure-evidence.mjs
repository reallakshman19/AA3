...
function targetRow(rows, nodeId, component) { const hits = rows.filter((r) => r.entityKind === 'NODE' && String(r.entityId) === String(nodeId) && r.quantity === 'FORCE' && r.component === component); if (hits.length !== 1) throw new Error(`Expected one target ${nodeId}/${component}; found ${hits.length}`); return hits[0]; }
...
const result = { schema:'lfea-m047-i031-reducer-pressure-evidence/v1', issueId:'M047', iterationId:'M047-I031', cases:{} };
for (const caseId of ['L19','L20']) {
  ...
  const targets = {};
  for (const [node,comp] of caseId==='L19' ? [['20090','FY'],['21470','FY']] : [['20030','FX']]) {
    const b=targetRow(br.comparison.rows,node,comp), a=targetRow(cr.comparison.rows,node,comp);
    targets[`${node}_${comp}`]={referenceValue:b.referenceValue,beforeValue:b.actualValue,beforeRelativeError:b.relativeError,beforeStatus:b.status,afterValue:a.actualValue,afterRelativeError:a.relativeError,afterStatus:a.status};
  }
...
for (const caseId of ['L19','L20']) {
  ...
  for (const [k,t] of Object.entries(c.targets)) lines.push(`- target ${k}: ${(100*Number(t.beforeRelativeError)).toFixed(4)}% (${t.beforeStatus}) -> ${(100*Number(t.afterRelativeError)).toFixed(4)}% (${t.afterStatus})`);
  lines.push('');
}
