...
CURRENT_ENDPOINT: EP-0009

## Original task / acceptance ledger

TASK-001 | Governance gate assertion repair and full meshing-chain execution. | IMPLEMENTED_EXECUTION_PENDING | original panel repair PASS on Owner runner; subsequent stale qualifier boundaries isolated through LEG-004; post-LEG-004 execution pending
...
INPUT-002 | Live main `b39f7673737bd1f7f4a6d7dd9d1538f795874281`. | AVAILABLE | latest observed drift remains disjoint from BM-MESH qualifier and inspected product-refinement surfaces
...
INPUT-006 | LAFEA.4 curved-shell local refinement now dispatches through a bounded TECH-13 product-refinement route; LAFEA.5 retains the generic route. | AVAILABLE | production workbench dispatch + product-refinement scope/UI policy
INPUT-007 | Owner executable runner evidence. | AVAILABLE_PARTIAL | post-LEG-003 focused/aggregate failures supplied; post-LEG-004 rerun pending
INPUT-008 | M4 frozen physics fixture authority. | UNRESOLVED | issue does not fully specify material/load/support fixture

## Benchmark / oracle ledger

BM-001 | `npm run check:lafea-meshing`. | PATCHED_NOT_RUN | latest run again reaches determinism; LEG-004 pending rerun
BM-002 | M0 producer conformance. | NOT_RUN | staged after TASK-001
BM-003 | M1 determinism. | PARTIAL_FAILED_PREPATCH / PATCHED_NOT_RUN | imported curved-shell qualifier refinement assertions isolated; current patch pending rerun
...
ACTIVE_QUESTION_SET: agents/qualifications/ADV-BM-MESH-1652/QS-ADV-BM-MESH-1652-0003-questions.md
...
The Owner post-LEG-003 rerun confirms parent curved-shell mesh generation, source authority and solver/compiler disposition reach the intended state, then the old qualifier invokes local refinement. LAFEA.4 no longer uses the historical generic local-refinement route for retained shell midsurfaces: cylindrical and cylindrical-hole surfaces enter a separate TECH-13 product candidate path, while periodic cylinders are excluded by that product scope. The producer/determinism qualifiers already observe refinement disabled, so LEG-004 verifies that current UI disposition without invoking the disabled LAFEA.4 product action. LAFEA.5 retains the generic rejected-refinement assertion. The separate TECH-13 graded-refinement point-contract defect is not repaired by BM-MESH.
