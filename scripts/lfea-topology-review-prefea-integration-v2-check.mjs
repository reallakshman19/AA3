...
console.log(`P02B2-01 PASS retained topology/proximity findings project into deterministic review queues ${review.semanticHash}`);
...
console.log('P02B2-02 PASS retained native-mm distances/tolerances are explicitly converted to metres without magnitude inference');
...
console.log('P02B2-03 PASS identical numeric evidence under explicit inch authority converts by 0.0254 m/in and changes review identity');

const metres = createLfeaTopologyReviewFromDiagnostics(diagnosticsFixture('m'));
const metreNearNode = metres.findings.find((row) => row.findingId.includes('NEAR_COINCIDENT'));
assert.equal(metreNearNode.distanceM, 0.5);
assert.equal(metreNearNode.toleranceM, 1);
console.log('P02B2-04 PASS explicit metre authority remains identity conversion without magnitude inference');
...
console.log('P02B2-05 PASS substituted parent evidence and unsupported measured geometry units fail closed');
...
console.log('P02B2-06 PASS finding identity/code/effect/source scope and diagnostic semantic/evidence hashes remain auditable');
...
console.log('P02B2-07 PASS read-only review UI exposes queues, measurement basis, source trace and explicit no-mutation policy');
...
console.log('P02B2-08 PASS normal P-08 diagnostics composition projects the same retained evidence into P-02 review without another detector');
...
console.log('P02B2-09 PASS P-02B owns no parser, detector rerun, topology mutation, solver, event-bus or authorization authority');

console.log(JSON.stringify({
  check: 'lfea-topology-review-prefea-integration-v2',
  status: 'PASS',
