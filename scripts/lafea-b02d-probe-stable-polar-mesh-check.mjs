...
import {
  ...
  lafeaB02dProbeStablePolarPolicy,
} from '../src/core/lafea-meshing/b02d-probe-stable-polar-mesh.js';
...
assert.equal(policy.designId, frozen.fixedProbeMeshPolicy.designId);
...
for (const method of ['T3', 'T6', 'Q8']) {
  for (const level of frozen.globalResponseLadder.levels) {
    const profile = meshProfile(method, level.h);
    const configuration = lafeaMeshGenerationConfiguration(profile, {
      refinementFeatureIds: [LAFEA_B02D_PROBE_STABLE_POLAR_FEATURE_ID],
    });
...
const badProfile = meshProfile('T6', 30);
assert.throws(
  () => planLafeaAnalysisMesh(stage, lafeaMeshGenerationConfiguration(badProfile, {
    refinementFeatureIds: [LAFEA_B02D_PROBE_STABLE_POLAR_FEATURE_ID],
  })),
...
assert.throws(
  () => planLafeaAnalysisMesh(rectangleStage, lafeaMeshGenerationConfiguration(meshProfile('T6', 20), {
    refinementFeatureIds: [LAFEA_B02D_PROBE_STABLE_POLAR_FEATURE_ID],
  })),
  (error) => error?.code === 'LAFEA_B02D_POLAR_GEOMETRY_NOT_QUALIFIED',
);
assert.throws(
  () => planLafeaAnalysisMesh(stage, lafeaMeshGenerationConfiguration(meshProfile('T6', 20), {
    refinementFeatureIds: [LAFEA_B02D_PROBE_STABLE_POLAR_FEATURE_ID, 'EXTRA'],
  })),
  (error) => error?.code === 'LAFEA_B02D_POLAR_RESERVED_FEATURE_COMBINATION_NOT_QUALIFIED',
);
...
function annulusGeometry() {
  return createLafeaAnalysisGeometry({
    ...
    geometryId: 'B02D-ANNULUS', coordinateSystemId: 'GLOBAL_XY', lengthUnit: 'mm',
    ...
    segments: [
      arc('O1','OE','ON',100,'CCW'), arc('O2','ON','OW',100,'CCW'),
      arc('O3','OW','OS',100,'CCW'), arc('O4','OS','OE',100,'CCW'),
      arc('H1','HE','HS',20,'CW'), arc('H2','HS','HW',20,'CW'),
      arc('H3','HW','HN',20,'CW'), arc('H4','HN','HE',20,'CW'),
    ],
    loops: [
...
function rectangleGeometry() {
  return createLafeaAnalysisGeometry({
    ...
    segments:[line('S1','A','B'),line('S2','B','C'),line('S3','C','D'),line('S4','D','A')],
...
function assertPath(mesh, ids, angleDegrees) {
  ...
  const radii = ids.map((id) => {
    const node = nodes.get(id); assert.ok(node,id);
    const cross = node.x*uy - node.y*ux;
    assert.ok(Math.abs(cross) <= 1e-9*Math.max(1,Math.hypot(node.x,node.y)), `${id} not on ${angleDegrees}`);
    return node.x*ux + node.y*uy;
  });
  assert.ok(Math.abs(radii[0]-20) <= 1e-9);
  assert.ok(Math.abs(radii.at(-1)-60) <= 1e-9);
...
function assertPhysicalCircularMidsides(mesh, method) {
  ...
  for (const element of mesh.elements) {
    const corners = element.nodeIds.slice(0, method==='Q8'?4:3);
    const mids = element.nodeIds.slice(method==='Q8'?4:3);
    const edges = method==='Q8'
      ? [[0,1,0],[1,2,1],[2,3,2],[3,0,3]]
      : [[0,1,0],[1,2,1],[2,0,2]];
    for (const [ia,ib,im] of edges) {
      const a=nodes.get(corners[ia]), b=nodes.get(corners[ib]), m=nodes.get(mids[im]);
      const ra=Math.hypot(a.x,a.y), rb=Math.hypot(b.x,b.y);
      if (Math.abs(ra-rb)<=1e-10 && (Math.abs(ra-20)<=1e-10 || Math.abs(ra-100)<=1e-10)) {
        assert.ok(Math.abs(Math.hypot(m.x,m.y)-ra)<=1e-9); count+=1;
      }
    }
  }
  assert.ok(count>0);
