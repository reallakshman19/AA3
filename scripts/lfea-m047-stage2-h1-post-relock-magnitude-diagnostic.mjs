...
async function runDiagnostic(input){
  ...
  for(const [nodeId,r] of [...last.entries()].sort()){
    ...
    // changes the scalar force along d by -k(Δu·d). Therefore:
    //   F_scalar = cap - k (Δu · d)
    // The previous diagnostic projected on -d but kept the same minus sign, which
    // inverted this scalar counterfactual. This corrected expression is data-only
    // and does not change any H1/H2 nonlinear solve or measured result.
