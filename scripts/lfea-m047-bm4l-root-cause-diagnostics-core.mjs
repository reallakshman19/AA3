...
function buildLinearityAuthority(actual) {
  ...
  for (const caseId of EXPECTED_CASES) {
    ...
    cases[caseId] = Object.freeze({
      ...
      modelCoefficientOfFriction: evidence.effectiveConfiguration?.modelCoefficientOfFriction ?? null,
      frictionMultiplier: evidence.frictionAuthority?.frictionMultiplier ?? null,
      effectiveCoefficientOfFriction: evidence.frictionAuthority?.effectiveCoefficient ?? null,
