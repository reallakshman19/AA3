...
for (const station of definitions.B02D.fixedPath.stations) assert.ok(['HIGH_GRADIENT_CONVERGENCE','NON_SINGULAR_CONVERGENCE'].includes(station.singularityClassification));
...
for (const definition of Object.values(definitions)) {
  for (const source of definition.independentOracle?.sourceRefs ?? definition.historicalDefinitionSources ?? []) assert.ok(fs.existsSync(path.join(ROOT, source)), `missing frozen source ${source}`);
}
