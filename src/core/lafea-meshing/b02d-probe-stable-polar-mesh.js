...
export const LAFEA_B02D_PROBE_STABLE_POLAR_FEATURE_ID = 'B02D_PROBE_STABLE_POLAR_V1';
export const LAFEA_B02D_PROBE_STABLE_POLAR_STRATEGY = 'B02D_PROBE_STABLE_POLAR';
export const LAFEA_B02D_PROBE_STABLE_POLAR_POLICY_ID = 'B02D_PROBE_STABLE_POLAR_POLICY_V1';

const H_LEVELS = Object.freeze([40, 20, 10, 5]);
const POLICY = deepFreeze({
  schema: 'lafea-b02d-probe-stable-polar-policy/v1',
  policyId: LAFEA_B02D_PROBE_STABLE_POLAR_POLICY_ID,
...
export function lafeaB02dProbeStablePolarPolicy() {
  return POLICY;
}

export function generateLafeaB02dProbeStablePolarMesh({ targetElementLength, elementFamily }) {
  const ordinal = ordinalForH(targetElementLength);
  if (!['T3', 'T6', 'Q8'].includes(elementFamily)) fail('LAFEA_B02D_POLAR_ELEMENT_FAMILY_INVALID');
  ...
  const base = {
    ...
    strategy: LAFEA_B02D_PROBE_STABLE_POLAR_STRATEGY,
    strategyReason: 'FROZEN_PHYSICAL_PROBE_STABLE_POLAR_POLICY',
    holeCount: 1,
  ...
  return deepFreeze({
    ...base,
    semanticHash: canonicalLafeaSha256({ schema: 'lafea-b02d-probe-stable-polar-mesh-output/v1', output: base }),
  });
}
...
function midpointId(state, firstId, secondId) {
  ...
  if (a?.kind === 'CORNER' && b?.kind === 'CORNER' && a.ring === b.ring
    ...
    if (!(norm > 0)) fail('LAFEA_B02D_POLAR_CIRCUMFERENTIAL_MIDSIDE_INVALID');
    point = {
...
function buildFeatureMapping(state) {
  ...
  if (load.nodeIds.length < 2 || restraint.nodeIds.length < 2) fail('LAFEA_B02D_POLAR_FEATURE_PATH_EMPTY');
  return deepFreeze({
    schema: 'lafea-b02d-probe-stable-polar-feature-mapping/v1',
...
function radialPath(state, angleDegrees) {
  ...
  if (sector < 0) fail('LAFEA_B02D_POLAR_CARDINAL_FEATURE_MISSING');
  ...
  if (start < 0 || end <= start) fail('LAFEA_B02D_POLAR_RADIAL_WINDOW_MISSING');
  const nodeIds = [];
  const edges = [];
...
function analysisMesh(state) {
  return deepFreeze({
    ...
    meshIdentity: `B02D-${state.elementFamily}-L${state.ordinal}-PROBE-STABLE-POLAR`,
...
function characteristicLengths(mesh) {
  ...
  if (!values.length) fail('LAFEA_B02D_POLAR_CHARACTERISTIC_LENGTH_EMPTY');
  return values;
}

function ordinalForH(value) {
  if (!Number.isFinite(value)) fail('LAFEA_B02D_POLAR_TARGET_H_INVALID');
  const index = H_LEVELS.findIndex((h) => Math.abs(h - value) <= 1e-12 * Math.max(1, h));
  if (index < 0) fail('LAFEA_B02D_POLAR_TARGET_H_NOT_FROZEN_LEVEL');
  return index + 1;
}
...
function addNode(state, nodeId, point, meta) {
  if (state.nodes.has(nodeId)) fail('LAFEA_B02D_POLAR_NODE_DUPLICATE');
...
function requireNode(state, nodeId) {
  ...
  if (!node) fail('LAFEA_B02D_POLAR_NODE_MISSING');
  return node;
}
function requirePositiveCorners(state, ids, elementId) {
  ...
  if (!(twice > 0)) fail(`LAFEA_B02D_POLAR_ELEMENT_ORIENTATION_INVALID:${elementId}`);
}
