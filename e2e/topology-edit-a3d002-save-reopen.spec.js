...
test('A3D-002 visible support edit survives exact Undo Redo Save Exit Reopen Reload', async ({ page }, testInfo) => {
  ...
  assertEngineeringAuthority(expect, await authorityEvidence(page), baseline);
  ...
  assertEngineeringAuthority(expect, staged, baseline);
  ...
  assertEngineeringAuthority(expect, previewed, baseline);
  ...
  assertEngineeringAuthority(expect, validated, baseline);
  ...
  expect(redone.sourceHash).toBe(baseline.sourceHash);
  expect(redone.sourceByteHash).toBe(baseline.sourceByteHash);
  ...
  expect(destroyed.sessionIsNull).toBe(true);
  expect(destroyed.viewportBackendIsNull).toBe(true);
  ...
  await testInfo.attach('a3d-002-save-reopen-ledger', {
    body: JSON.stringify(evidence, null, 2),
    contentType: 'application/json',
  });
  await testInfo.attach('a3d-002-save-reopen', {
...
async function destroyedEditorEvidence(page) {
  return page.evaluate(() => {
    ...
    const controller = host?.__topologyEditAuthoringController;
    return {
      sessionIsNull: controller?.session === null,
      viewportBackendIsNull: controller?.viewportBackend === null,
...
function assertEngineeringAuthority(expectApi, actual, expected) {
  expectApi(actual.canonicalHash).toBe(expected.canonicalHash);
  expectApi(actual.journalHash).toBe(expected.journalHash);
  expectApi(actual.activeLedgerHash).toBe(expected.activeLedgerHash);
  expectApi(actual.activeCommandIds).toEqual(expected.activeCommandIds);
  expectApi(actual.activeCommandCount).toBe(expected.activeCommandCount);
  expectApi(actual.sessionVersion).toBe(expected.sessionVersion);
