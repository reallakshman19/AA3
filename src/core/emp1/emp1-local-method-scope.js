...
function validateGamma5Runtime(scope,local,reasons){
  ...
  if(local.loadReference!==WRC_LOAD_REFERENCE) reasons.push('EMP1_LOCAL_METHOD_GAMMA5_RUNTIME_LOAD_REFERENCE_INVALID');
  if(local.pressureThrustDisposition!==UPSTREAM_PRESSURE_DISPOSITION) reasons.push('EMP1_LOCAL_METHOD_GAMMA5_RUNTIME_PRESSURE_DISPOSITION_INVALID');
  if(!nonEmpty(local.sourceLoadCustodyHash)) reasons.push('EMP1_LOCAL_METHOD_GAMMA5_RUNTIME_LOAD_CUSTODY_HASH_REQUIRED');
  if(reasons.some((reason)=>reason==='EMP1_LOCAL_METHOD_SCOPED_RUNTIME_GAMMA_INVALID')) return;
  const domain=evaluateEmp1Wrc537BoundedDomain({
    shellFamily:local.shellFamily,
    attachmentShape:local.attachmentShape,
    sourceDocumentSha256:local.sourceSha256,
    datasetHash:local.datasetHash,
    variant:local.variant,
    gamma:local.gamma,
    beta:local.beta,
  });
  if(domain.status!=='PASS_BOUNDED_DOMAIN'){
    for(const reason of domain.reasons) reasons.push(`EMP1_LOCAL_METHOD_GAMMA5_RUNTIME_DOMAIN:${reason}`);
  }
}

function roundOffEquivalent(actual, source, relativeTolerance) {
  return Number.isFinite(actual)&&Number.isFinite(source)&&Math.abs(actual - source) <= Math.max(1, Math.abs(source)) * relativeTolerance;
}
