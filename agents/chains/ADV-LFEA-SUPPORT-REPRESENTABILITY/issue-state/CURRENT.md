...
TASK-002 | CNODE terminal refusal closure | PARTIAL | finite bidirectional CNODE spring lands exact DRAFT; rigid CNODE deliberately refused pending MPC; exact topology subgate PASS/red but full production mechanics unexecuted
...
INPUT-018 | Spring-rate resolver guard | EXECUTED_EXACT_HEAD_GIT_BLOB_LOCAL | two exact blobs `126d25ce...` + `50094a2b...`; normal PASS, raw-rate red and unresolved-factor-1 red observed
INPUT-019 | Broad spring-rate structural-unit guard | AVAILABLE_SOURCE_NOT_RUN | `lfea-spring-rate-units-check.mjs` now has deliberate breaks but its structural compiler closure is not yet exact-executed
...
BM-003 | Exact-head deliberate-break red observations | PARTIAL | reaction aggregation, mixed unilateral, HANGER case/run, blocked-execution, CNODE topology and two spring-rate resolver falsifiers observed red; dedicated refusal and feature production-mechanics breaks still NOT_RUN
...
BM-009 | Dedicated three-feature refusal fixture gate | NOT_RUN | source custody authored/wired with deliberate-break mode; full InputXML dependency closure not yet materialized
...
BM-012 | Spring-rate resolver unit guard | PASS_EXACT_HEAD_GIT_BLOB_LOCAL | N/m=1, N/cm=100, N/mm=1000, lbf/in=175.12683524647636; 400 N/mm→400000 N/m; unresolved→null; raw-rate and factor-1 fallback breaks both red
BM-013 | Broad spring-rate structural fail-closed guard | NOT_RUN | deliberate-break source authored; structural compiler dependency closure not yet materialized

## Exact-head execution provenance

Local runtime: Node v22.16.0.

Previously admitted exact evidence remains recorded in EP-0023 through EP-0025.

### Spring-rate resolver closure

Test head: `ef275dfea07000b2dd3acaba4c462443c231cade`.
...
Both local files reproduced their repository Git blob SHA before execution. Isolated execution used `node --experimental-default-type=module` to declare the same ESM interpretation as the repository module mode without relying on an unverified package file.

Observed normal: exit 0/PASS; factors N/m=1, N/cm=100, N/mm=1000, lbf/in=175.12683524647636; declared 400 N/mm resolves to 400000 N/m; unresolved units retain declared 400 but withhold usable value (`null`).

Observed raw-rate break: exit 1 at `400 N/mm must resolve to 400000 N/m, never raw 400 N/m`; actual `400`, expected `400000`.

Observed unresolved fallback break: exit 1 at `unresolved units must withhold the solver stiffness rather than assume factor 1`; actual `400`, expected `null`.

`614d274e... -> ef275dfe...` changes only `lfea-spring-rate-units-check.mjs` and the new resolver check; production `restraint-spring-rate.js` remains unchanged.
...
DRIFT-003 | EP-0025 head `614d274e... -> ef275dfe...` | TEST_ONLY | two test files only; no production numerical authority changed
...
Current question set: `QS-ADV-LFEA-SUPPORT-REPRESENTABILITY-0026`.
Qualification basis: `ef275dfea07000b2dd3acaba4c462443c231cade`.
The original five issue challenges remain controlling. QS-0026 promotes only the resolver-level Q4 unit conversion/withholding evidence and preserves the broader structural fail-closed gate as NOT_RUN.

## Current blockers

1. Dedicated refusal gate + break, mixed fixed+skew production check + break, finite skew/CNODE/HANGER production-mechanics checks, remaining deliberate breaks, support/full aggregates, imports/lint/diff and BM4_L remain NOT_RUN.
2. Broad structural unit gate `INPUTXML_STRUCTURAL_SPRING_RATE_UNRESOLVED` remains NOT_RUN even though its deliberate-break source is now authored.
3. Independent CAESAR support references REF-SKEW-01 / REF-CNODE-01 / REF-HGR-01 remain absent.
4. Rigid skew/CNODE still require exact MPC/constraint-equation authority; alternate hanger vertical axis remains source-authority gated.
5. Live-main LAFEA/InputXML drift remains material outside the isolated exact micro-gates; broader InputXML promotion must reconcile complete dependency sets.
