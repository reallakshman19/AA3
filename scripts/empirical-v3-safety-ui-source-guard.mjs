...
const controller=sourceAt('src/workspace/empirical-v3-safety-workbench.js');assert.match(controller,/createEngineeringConfirmationReceipt/);assert.match(controller,/Safety Gate remains unchanged until a re-evaluated sealed package is supplied/);assert.match(controller,/VIEWPORT_SELECTION_REQUESTED/);assert.match(controller,/isRunReady\(\)/);assert.match(controller,/prepared execution custody/);assert.match(controller,/restoreDownstream/);assert.match(controller,/CALCULATION_EVIDENCE/);assert.match(controller,/RESULT_REVIEW/);assert.match(controller,/AUDIT_READINESS/);assert.match(controller,/workflow\.state!=='RESULT_REVIEW_REQUIRED'/);assert.match(controller,/workflow\.state!=='RESULT_REVIEWED'/);assert.match(controller,/Audit readiness is a separate governed transition/);assert.match(controller,/onAuditExportRequested/);assert.match(controller,/evidence\.authorizationRef\.semanticHash===packageValue\.calculationAuthorization\.semanticHash/);
const main=sourceAt('src/main.js');assert.match(main,/mountEmpiricalV3SafetyWorkbench/);assert.match(main,/prepareEmpiricalV3SourceBoundExecution/);assert.match(main,/buildEmpiricalV3SourceBoundExecutionDependency/);assert.match(main,/executeEmpiricalV3LiveSourceBoundRun/);assert.match(main,/applyEmpiricalV3LiveResultReview/);assert.match(main,/applyEmpiricalV3LiveAuditReadiness/);assert.match(main,/reconcileEmpiricalV3LiveAuthorization/);assert.match(main,/onRunRequested/);assert.match(main,/isRunReady/);assert.match(main,/Prepared source-bound execution request is not in the sealed calculation authorization/);assert.doesNotMatch(main,/rooted-tree-component-flexibility|restraint-compatibility|calculateCircularElbow|solveRootedTree/i);
