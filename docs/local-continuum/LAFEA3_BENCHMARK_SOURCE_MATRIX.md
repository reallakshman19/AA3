...
| `LAFEA3-SRC-LAME-01` | `scripts/lafea.3-benchmark-cont-cyl-01-check.mjs` | Classical Lamé thick-cylinder solution. Engineering cross-reference: R. H. MacNeal & R. L. Harder (1985), **Fig. 10, p. 10**, thick-walled-cylinder FE benchmark; for the analytical equations use the Lamé form `sigma_r=A-B/r²`, `sigma_theta=A+B/r²`. A book locator retained for classical mechanics custody: E. P. Popov, *Mechanics of Materials*, 2nd ed., Prentice-Hall, 1976, thick-walled-cylinder treatment (the repository does not depend on a secondary web transcription for numerical values). | Elastic thick-cylinder radial and hoop stresses vary as `A ∓ B/r²`, with constants fixed by radial-pressure boundary conditions. | Repository BVP: `Ri=50 mm`, `Ro=100 mm`, `Pi=10 MPa`, `Po=0`. Independent constants: `A=Pi*Ri²/(Ro²-Ri²)=10/3 MPa`; `B=Pi*Ri²*Ro²/(Ro²-Ri²)=33333.333333 MPa·mm²`. Therefore `sigma_r(Ri)=-10 MPa`, `sigma_r(Ro)=0`, `sigma_theta(Ri)=16.6666667 MPa`, `sigma_theta(Ro)=6.6666667 MPa`. MacNeal–Harder Fig. 10 is an independent FE benchmark family reference but has different radii/material/formulation, so it is **not** claimed as the exact fixture. | `QUALIFIED_CLASSICAL_ORACLE / ADAPTED_FE_BENCHMARK_FAMILY` |
...
The benchmark compares the full Q8 Gauss-point field after Cartesian-to-polar stress transformation; the published analytical formula, not the FE output, owns the expected field.
