...
const readyDataset = {
  ...
  equations: [
    ...
    { id: 'EQ_STRESS_INTENSITY', machine: '0.5*((sigx+sigy)+sqrt((sigx-sigy)**2+4*tau**2))' },
  ],
};
...
dimensionallyWrongDataset.equations.find((row) => row.id === 'EQ_STRESS_INTENSITY').machine = 'sqrt(0.5*((sigx+sigy)+sqrt((sigx-sigy)**2+4*tau**2)))';
...
assert.equal(dimensional.metrics.dimensionalContract.violations.length, 3);
...
const missingStressIntensity = structuredClone(readyDataset);
missingStressIntensity.equations = missingStressIntensity.equations.filter((row) => row.id !== 'EQ_STRESS_INTENSITY');
assert.deepEqual(
  inspectWrcMethodDimensionalContract(missingStressIntensity).violations.map((row) => row.id),
  ['STRESS_INTENSITY_EQUATION_MISSING'],
);
...
console.log(JSON.stringify({
  schema: 'emp1-wrc-dataset-readiness-self-test/v4',
  status: 'PASS',
  ...
  verifiedGuardrails: [
    ...
    'retained stress-intensity transcription cannot apply an outer square-root to a first-degree stress expression',
